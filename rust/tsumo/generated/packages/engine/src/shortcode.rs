...
impl ParseState {
    ...
    pub fn peek_string(&self, length: i32) -> rt::TsonicResult<String> {
        ...
        let slice_length: f64 = if length < remaining {
            tsonic_rust_runtime::conversions::i32_to_f64(length)
        } else {
            tsonic_rust_runtime::conversions::i32_to_f64(remaining)
        };
        crate::utils::strings::substring_count(
            self.state.with(|state| state.text.clone()),
            self.state.with(|state| state.pos),
            tsonic_rust_runtime::conversions::f64_to_i32(slice_length)?,
        )
    }
    ...
    pub fn is_in_code_block(&self, offset: i32) -> rt::TsonicResult<bool> {
        ...
        while low <= high {
            ...
            if offset < range.state.with(|state| state.start) {
                ...
                if offset >= range.state.with(|state| state.end) {
                    low = middle + 1;
                } else {
                    return Ok(true);
                }
