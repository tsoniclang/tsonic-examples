...
pub fn render_hook_template(
    ...
    > = js_abi::JsMap::new();
    {
        ...
        dispatch_receiver.dispatch.clone().dispatch_template_environment_render_template(
            template,
            value,
    ...
    let count: i32 = document.occurrence_count();
    {
        ...
        'loop_value: while index >= 0 {
            ...
            if occurrence.state.with(|state| state.kind.clone()) == "image" {
                let template: Option<crate::template::template_2::Template> = context
                    .state
                    .with(|state| state.image_hook.clone());
                if template.is_none() {
                    index -= 1;
                    continue 'loop_value;
                }
            ...
            if occurrence.state.with(|state| state.kind.clone()) == "link" {
                let template: Option<crate::template::template_2::Template> = context
                    .state
                    .with(|state| state.link_hook.clone());
                if template.is_none() {
                    index -= 1;
                    continue 'loop_value;
                }
