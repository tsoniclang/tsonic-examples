...
pub fn collect_page_bundle_resource_files(
    ...
    {
        let mut index: f64 = 0.0;
        'loop_value_2: while index < (tsonic_rust_runtime::conversions::usize_to_i32(directories.len())? as f64) {
            ...
            if is_nested_bundle(child.clone())?
                || tsonic_rust_runtime::conversions::usize_to_i32(
                    crate::fs::list_files_top_directory(child.clone(), String::from("*.md"))?
                        .len(),
                )? > 0
            {
                index += 1.0;
