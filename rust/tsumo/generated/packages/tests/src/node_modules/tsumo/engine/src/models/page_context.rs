// Generated by the Tsonic Rust target. Do not edit.

use tsonic_rust_js::abi as js_abi;

use crate::program as rt;

#[allow(dead_code, reason = "preserves the checked source contract")]
pub(crate) struct PageContextState {
    pub(crate) title: String,
    pub(crate) date: String,
    pub(crate) lastmod: String,
    pub(crate) draft: bool,
    pub(crate) kind: String,
    pub(crate) section: String,
    pub(crate) r#type: String,
    pub(crate) slug: String,
    pub(crate) rel_permalink: String,
    pub(crate) plain: String,
    pub(crate) table_of_contents: crate::node_modules::tsumo::engine::src::utils::html::HtmlString,
    pub(crate) content: crate::node_modules::tsumo::engine::src::utils::html::HtmlString,
    pub(crate) summary: crate::node_modules::tsumo::engine::src::utils::html::HtmlString,
    pub(crate) description: String,
    pub(crate) tags: js_abi::JsArray<String>,
    pub(crate) categories: js_abi::JsArray<String>,
    pub(crate) params:
        js_abi::JsMap<String, crate::node_modules::tsumo::engine::src::params::ParamValue>,
    pub(crate) file: Option<crate::node_modules::tsumo::engine::src::models::page_file::PageFile>,
    pub(crate) language: crate::node_modules::tsumo::engine::src::models::language::LanguageContext,
    pub(crate) translations: js_abi::JsArray<PageContext>,
    pub(crate) store:
        Option<crate::node_modules::tsumo::engine::src::template::values::scratch::ScratchStore>,
    pub(crate) site: crate::node_modules::tsumo::engine::src::models::site_context::SiteContext,
    pub(crate) pages: js_abi::JsArray<PageContext>,
    pub(crate) layout: Option<String>,
    pub(crate) parent: Option<PageContext>,
    pub(crate) ancestors: js_abi::JsArray<PageContext>,
    pub(crate) resource_source_dir: Option<String>,
    pub(crate) shortcode_names: js_abi::JsMap<String, bool>,
}

#[derive(Clone, Debug, PartialEq)]
pub struct PageContext {
    pub(crate) state: rt::ObjectHandle<PageContextState>,
}

impl PageContext {
    #[expect(clippy::too_many_arguments, reason = "checked source signature")]
    pub fn new(
        title: String,
        date: String,
        lastmod: String,
        draft: bool,
        kind: String,
        section: String,
        r#type: String,
        slug: String,
        rel_permalink: String,
        plain: String,
        table_of_contents: crate::node_modules::tsumo::engine::src::utils::html::HtmlString,
        content: crate::node_modules::tsumo::engine::src::utils::html::HtmlString,
        summary: crate::node_modules::tsumo::engine::src::utils::html::HtmlString,
        description: String,
        tags: js_abi::JsArray<String>,
        categories: js_abi::JsArray<String>,
        params: js_abi::JsMap<String, crate::node_modules::tsumo::engine::src::params::ParamValue>,
        file: Option<crate::node_modules::tsumo::engine::src::models::page_file::PageFile>,
        language: crate::node_modules::tsumo::engine::src::models::language::LanguageContext,
        translations: js_abi::JsArray<PageContext>,
        store: Option<
            crate::node_modules::tsumo::engine::src::template::values::scratch::ScratchStore,
        >,
        site: crate::node_modules::tsumo::engine::src::models::site_context::SiteContext,
        pages: js_abi::JsArray<PageContext>,
        parent: Option<PageContext>,
        ancestors: js_abi::JsArray<PageContext>,
        layout: Option<String>,
    ) -> PageContext {
        let field_title: String = title.clone();
        let field_date: String = date.clone();
        let field_lastmod: String = lastmod.clone();
        let field_draft: bool = draft;
        let field_kind: String = kind.clone();
        let field_section: String = section.clone();
        let field_r_type: String = r#type.clone();
        let field_slug: String = slug.clone();
        let field_rel_permalink: String = rel_permalink.clone();
        let field_plain: String = plain.clone();
        let field_table_of_contents: crate::node_modules::tsumo::engine::src::utils::html::HtmlString =
            table_of_contents.clone();
        let field_content: crate::node_modules::tsumo::engine::src::utils::html::HtmlString =
            content.clone();
        let field_summary: crate::node_modules::tsumo::engine::src::utils::html::HtmlString =
            summary.clone();
        let field_description: String = description.clone();
        let field_tags: js_abi::JsArray<String> = tags.clone();
        let field_categories: js_abi::JsArray<String> = categories.clone();
        let field_params: js_abi::JsMap<
            String,
            crate::node_modules::tsumo::engine::src::params::ParamValue,
        > = params.clone();
        let field_file: Option<
            crate::node_modules::tsumo::engine::src::models::page_file::PageFile,
        > = file.clone();
        let field_language: crate::node_modules::tsumo::engine::src::models::language::LanguageContext =
            language.clone();
        let field_translations: js_abi::JsArray<PageContext> = translations.clone();
        let field_store: Option<
            crate::node_modules::tsumo::engine::src::template::values::scratch::ScratchStore,
        > = store.clone();
        let field_site: crate::node_modules::tsumo::engine::src::models::site_context::SiteContext =
            site.clone();
        let field_pages: js_abi::JsArray<PageContext> = pages.clone();
        let field_layout: Option<String> = layout.clone();
        let field_parent: Option<PageContext> = parent.clone();
        let field_ancestors: js_abi::JsArray<PageContext> = ancestors.clone();
        let field_resource_source_dir: Option<String> = Option::<String>::None;
        let field_shortcode_names: js_abi::JsMap<String, bool> = js_abi::JsMap::new();
        PageContext {
            state: rt::ObjectHandle::new(PageContextState {
                title: field_title,
                date: field_date,
                lastmod: field_lastmod,
                draft: field_draft,
                kind: field_kind,
                section: field_section,
                r#type: field_r_type,
                slug: field_slug,
                rel_permalink: field_rel_permalink,
                plain: field_plain,
                table_of_contents: field_table_of_contents,
                content: field_content,
                summary: field_summary,
                description: field_description,
                tags: field_tags,
                categories: field_categories,
                params: field_params,
                file: field_file,
                language: field_language,
                translations: field_translations,
                store: field_store,
                site: field_site,
                pages: field_pages,
                layout: field_layout,
                parent: field_parent,
                ancestors: field_ancestors,
                resource_source_dir: field_resource_source_dir,
                shortcode_names: field_shortcode_names,
            }),
        }
    }
}
