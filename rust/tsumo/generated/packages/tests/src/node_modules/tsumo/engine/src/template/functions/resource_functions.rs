// Generated by the Tsonic Rust target. Do not edit.

use tsonic_rust_js::abi as js_abi;

use tsonic_rust_js::string as js_string;

use crate::program as rt;

pub(crate) fn resource_build_option(
    options: crate::node_modules::tsumo::engine::src::template::values::dict::DictValue,
    name: String,
) -> Option<crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue> {
    let exact: Option<
        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
    > = { let dispatch_receiver = &options; dispatch_receiver.dispatch.read_dict_value_value() }
        .get(&name);
    if exact.is_some() {
        return Some(match exact.as_ref() {
            Some(flow_value) => flow_value.clone(),
            None => unreachable!("checked flow selected a missing optional value"),
        });
    }
    let normalized: String = js_string::to_lower_case(&name);
    'loop_value: for key in
        { let dispatch_receiver_2 = &options; dispatch_receiver_2.dispatch.read_dict_value_value() }
            .keys()
    {
        if js_string::to_lower_case(&key) != normalized {
            continue 'loop_value;
        }
        return {
            let dispatch_receiver_3 = &options;
            dispatch_receiver_3.dispatch.read_dict_value_value()
        }
        .get(&key);
    }
    Option::<crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue>::None
}

pub(crate) fn validate_css_build_options(
    options: crate::node_modules::tsumo::engine::src::template::values::dict::DictValue,
) -> rt::TsonicResult<()> {
    'loop_value: for key in
        { let dispatch_receiver = &options; dispatch_receiver.dispatch.read_dict_value_value() }
            .keys()
    {
        let normalized: String = js_string::to_lower_case(&key);
        if normalized == "targetpath" || normalized == "minify" || normalized == "sourcemap" {
            continue 'loop_value;
        }
        return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
            String::from("TSUMO_TEMPLATE_CSS_BUILD_OPTION_UNKNOWN"),
            format!(
                "{}{}{}",
                String::from("css.Build does not support option '"),
                rt::source_string(&key),
                String::from("'"),
            ),
            None,
            None,
            None,
        )));
    }
    Ok(())
}

pub(crate) fn build_css_resource(
    manager: crate::node_modules::tsumo::engine::src::resources::manager::ResourceManager,
    source: crate::node_modules::tsumo::engine::src::resources::models::Resource,
    options: crate::node_modules::tsumo::engine::src::template::values::dict::DictValue,
) -> rt::TsonicResult<
    crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue,
> {
    validate_css_build_options(options.clone())?;
    let source_map: Option<
        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
    > = resource_build_option(options.clone(), String::from("sourceMap"));
    if source_map.is_some()
        && js_string::to_lower_case(&js_string::trim(
            &crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                .with(|module_binding| module_binding.load())
                .call((match source_map.as_ref() {
                    Some(flow_value) => flow_value.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },))?,
        )) != "none"
    {
        return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
            String::from("TSUMO_TEMPLATE_CSS_BUILD_SOURCE_MAP_UNSUPPORTED"),
            String::from("css.Build supports only sourceMap 'none'"),
            None,
            None,
            None,
        )));
    }
    let mut result: crate::node_modules::tsumo::engine::src::resources::models::Resource =
        source.clone();
    let target_path: Option<
        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
    > = resource_build_option(options.clone(), String::from("targetPath"));
    if target_path.is_some()
        && !js_string::trim(
            &crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                .with(|module_binding| module_binding.load())
                .call((match target_path.as_ref() {
                    Some(flow_value_2) => flow_value_2.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },))?,
        )
        .is_empty()
    {
        result = manager.copy(
            crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                .with(|module_binding| module_binding.load())
                .call((match target_path.as_ref() {
                    Some(flow_value_3) => flow_value_3.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },))?,
            result.clone(),
        )?;
    }
    let minify: Option<
        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
    > = resource_build_option(options.clone(), String::from("minify"));
    if minify.as_ref().is_some_and(|value| {
        value
            .dispatch
            .clone()
            .downcast_template_value_to_bool_value()
            .is_some()
    }) {
        #[expect(clippy::blocks_in_conditions, reason = "checked evaluation region")]
        if {
            let dispatch_receiver = &{
                let downcast_value = &minify;
                crate::node_modules::tsumo::engine::src::template::values::primitives::BoolValue {
                    identity: downcast_value.as_ref().unwrap().identity.clone(),
                    dispatch: downcast_value
                        .as_ref()
                        .unwrap()
                        .dispatch
                        .clone()
                        .downcast_template_value_to_bool_value()
                        .unwrap(),
                }
            };
            dispatch_receiver.dispatch.read_bool_value_value()
        } {
            result = manager.minify(result.clone())?;
        }
    } else {
        if minify.is_some() {
            return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
                String::from("TSUMO_TEMPLATE_CSS_BUILD_MINIFY_INVALID"),
                String::from("css.Build minify must be a boolean"),
                None,
                None,
                None,
            )));
        }
    }
    Ok(crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue::new(
        manager.clone(),
        result.clone(),
    ))
}

pub(crate) fn javascript_option_is_one_of(
    value: String,
    accepted: js_abi::JsArray<String>,
) -> rt::TsonicResult<bool> {
    {
        let mut index: f64 = 0.0;
        while index < (tsonic_rust_runtime::conversions::usize_to_i32(accepted.len())? as f64) {
            if value
                == (match accepted.get_number(index).as_ref() {
                    Some(flow_value) => flow_value.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                }) {
                return Ok(true);
            }
            index += 1.0;
        }
    }
    Ok(false)
}

pub(crate) fn serialize_java_script_build_params(
    value: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
) -> rt::TsonicResult<String> {
    if value
        .dispatch
        .clone()
        .downcast_template_value_to_dict_value()
        .is_some()
    {
        return crate::node_modules::tsumo::engine::src::template::evaluation::serialization::to_json(
            value.clone(),
        );
    }
    if value
        .dispatch
        .clone()
        .downcast_template_value_to_any_array_value()
        .is_some()
    {
        return crate::node_modules::tsumo::engine::src::template::evaluation::serialization::to_json(
            value.clone(),
        );
    }
    Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
        String::from("TSUMO_TEMPLATE_JAVASCRIPT_BUILD_PARAMS_INVALID"),
        String::from("js.Build params must be a dictionary or slice"),
        None,
        None,
        None,
    )))
}

pub(crate) fn parse_java_script_build_option_dictionary(
    value: crate::node_modules::tsumo::engine::src::template::values::dict::DictValue,
) -> rt::TsonicResult<
    crate::node_modules::tsumo::engine::src::resources::javascript_provider::JavaScriptBuildOptions,
> {
    let options: crate::node_modules::tsumo::engine::src::resources::javascript_provider::JavaScriptBuildOptions = crate::node_modules::tsumo::engine::src::resources::javascript_provider::JavaScriptBuildOptions::new();
    'loop_value: for key in
        { let dispatch_receiver = &value; dispatch_receiver.dispatch.read_dict_value_value() }
            .keys()
    {
        let normalized: String = js_string::to_lower_case(&key);
        if normalized == "targetpath"
            || normalized == "minify"
            || normalized == "format"
            || normalized == "target"
            || normalized == "platform"
            || normalized == "sourcemap"
            || normalized == "params"
            || normalized == "jsxfactory"
        {
            continue 'loop_value;
        }
        return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
            String::from("TSUMO_TEMPLATE_JAVASCRIPT_BUILD_OPTION_UNKNOWN"),
            format!(
                "{}{}{}",
                String::from("js.Build does not support option '"),
                rt::source_string(&key),
                String::from("'"),
            ),
            None,
            None,
            None,
        )));
    }
    let target_path: Option<
        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
    > = resource_build_option(value.clone(), String::from("targetPath"));
    if target_path.is_some() {
        {
            let receiver = &options;
            let value_2 = Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                    .with(|module_binding| module_binding.load())
                    .call((match target_path.as_ref() {
                        Some(flow_value) => flow_value.clone(),
                        None => unreachable!("checked flow selected a missing optional value"),
                    },))?,
            );
            receiver.state.with_mut(|state| state.target_path = value_2)
        };
    }
    let minify: Option<
        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
    > = resource_build_option(value.clone(), String::from("minify"));
    if minify.is_some() {
        if match minify.as_ref() {
            Some(flow_value_2) => flow_value_2.clone(),
            None => unreachable!("checked flow selected a missing optional value"),
        }
        .dispatch
        .clone()
        .downcast_template_value_to_bool_value()
        .is_none()
        {
            return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
                String::from("TSUMO_TEMPLATE_JAVASCRIPT_BUILD_MINIFY_INVALID"),
                String::from("js.Build minify must be a boolean"),
                None,
                None,
                None,
            )));
        }
        {
            let receiver_2 = &options;
            let value_3 = {
                let dispatch_receiver_2 = &{
                    let downcast_value = &minify;
                    crate::node_modules::tsumo::engine::src::template::values::primitives::BoolValue {
                        identity: downcast_value.as_ref().unwrap().identity.clone(),
                        dispatch: downcast_value
                            .as_ref()
                            .unwrap()
                            .dispatch
                            .clone()
                            .downcast_template_value_to_bool_value()
                            .unwrap(),
                    }
                };
                dispatch_receiver_2.dispatch.read_bool_value_value()
            };
            receiver_2.state.with_mut(|state| state.minify = value_3)
        };
    }
    let format: Option<
        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
    > = resource_build_option(value.clone(), String::from("format"));
    if format.is_some() {
        let selected: String = js_string::to_lower_case(&js_string::trim(
            &crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                .with(|module_binding| module_binding.load())
                .call((match format.as_ref() {
                    Some(flow_value_3) => flow_value_3.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },))?,
        ));
        if !javascript_option_is_one_of(
            selected.clone(),
            js_abi::JsArray::from_dense(vec![
                String::from("iife"),
                String::from("cjs"),
                String::from("esm"),
            ]),
        )?
        {
            return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
                String::from("TSUMO_TEMPLATE_JAVASCRIPT_BUILD_FORMAT_INVALID"),
                format!(
                    "{}{}{}",
                    String::from("js.Build format '"),
                    rt::source_string(&selected),
                    String::from("' is invalid"),
                ),
                None,
                None,
                None,
            )));
        }
        {
            let receiver_3 = &options;
            let value_4 = selected.clone();
            receiver_3.state.with_mut(|state| state.format = value_4)
        };
    }
    let target: Option<
        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
    > = resource_build_option(value.clone(), String::from("target"));
    if target.is_some() {
        let selected: String = js_string::to_lower_case(&js_string::trim(
            &crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                .with(|module_binding| module_binding.load())
                .call((match target.as_ref() {
                    Some(flow_value_4) => flow_value_4.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },))?,
        ));
        if !javascript_option_is_one_of(
            selected.clone(),
            js_abi::JsArray::from_dense(vec![
                String::from("es5"),
                String::from("es2015"),
                String::from("es2016"),
                String::from("es2017"),
                String::from("es2018"),
                String::from("es2019"),
                String::from("es2020"),
                String::from("es2021"),
                String::from("es2022"),
                String::from("es2023"),
                String::from("es2024"),
                String::from("esnext"),
            ]),
        )?
        {
            return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
                String::from("TSUMO_TEMPLATE_JAVASCRIPT_BUILD_TARGET_INVALID"),
                format!(
                    "{}{}{}",
                    String::from("js.Build target '"),
                    rt::source_string(&selected),
                    String::from("' is invalid"),
                ),
                None,
                None,
                None,
            )));
        }
        {
            let receiver_4 = &options;
            let value_5 = selected.clone();
            receiver_4.state.with_mut(|state| state.target = value_5)
        };
    }
    let platform: Option<
        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
    > = resource_build_option(value.clone(), String::from("platform"));
    if platform.is_some() {
        let selected: String = js_string::to_lower_case(&js_string::trim(
            &crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                .with(|module_binding| module_binding.load())
                .call((match platform.as_ref() {
                    Some(flow_value_5) => flow_value_5.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },))?,
        ));
        if !javascript_option_is_one_of(
            selected.clone(),
            js_abi::JsArray::from_dense(vec![
                String::from("browser"),
                String::from("node"),
                String::from("neutral"),
            ]),
        )?
        {
            return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
                String::from("TSUMO_TEMPLATE_JAVASCRIPT_BUILD_PLATFORM_INVALID"),
                format!(
                    "{}{}{}",
                    String::from("js.Build platform '"),
                    rt::source_string(&selected),
                    String::from("' is invalid"),
                ),
                None,
                None,
                None,
            )));
        }
        {
            let receiver_5 = &options;
            let value_6 = selected.clone();
            receiver_5.state.with_mut(|state| state.platform = value_6)
        };
    }
    let source_map: Option<
        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
    > = resource_build_option(value.clone(), String::from("sourceMap"));
    if source_map.is_some() {
        {
            let receiver_6 = &options;
            let value_7 = js_string::to_lower_case(&js_string::trim(
                &crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                    .with(|module_binding| module_binding.load())
                    .call((match source_map.as_ref() {
                        Some(flow_value_6) => flow_value_6.clone(),
                        None => unreachable!("checked flow selected a missing optional value"),
                    },))?,
            ));
            receiver_6
                .state
                .with_mut(|state| state.source_map = value_7)
        };
    }
    let params: Option<
        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
    > = resource_build_option(value.clone(), String::from("params"));
    if params.is_some() {
        {
            let receiver_7 = &options;
            let value_8 = Some(serialize_java_script_build_params(match params.as_ref() {
                Some(flow_value_7) => flow_value_7.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            })?);
            receiver_7
                .state
                .with_mut(|state| state.params_json = value_8)
        };
    }
    let jsx_factory: Option<
        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
    > = resource_build_option(value.clone(), String::from("JSXFactory"));
    if jsx_factory.is_some() {
        {
            let receiver_8 = &options;
            let value_9 = Some(js_string::trim(
                &crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                    .with(|module_binding| module_binding.load())
                    .call((match jsx_factory.as_ref() {
                        Some(flow_value_8) => flow_value_8.clone(),
                        None => unreachable!("checked flow selected a missing optional value"),
                    },))?,
            ));
            receiver_8
                .state
                .with_mut(|state| state.jsx_factory = value_9)
        };
    }
    Ok(options.clone())
}

pub(crate) fn parse_java_script_build_options(
    value: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
) -> rt::TsonicResult<
    crate::node_modules::tsumo::engine::src::resources::javascript_provider::JavaScriptBuildOptions,
> {
    if value
        .dispatch
        .clone()
        .downcast_template_value_to_string_value()
        .is_some()
    {
        let options: crate::node_modules::tsumo::engine::src::resources::javascript_provider::JavaScriptBuildOptions = crate::node_modules::tsumo::engine::src::resources::javascript_provider::JavaScriptBuildOptions::new();
        {
            let receiver = &options;
            let value_2 = Some({
                let dispatch_receiver = &{
                    let downcast_value = &value;
                    crate::node_modules::tsumo::engine::src::template::values::primitives::StringValue {
                        identity: downcast_value.identity.clone(),
                        dispatch: downcast_value
                            .dispatch
                            .clone()
                            .downcast_template_value_to_string_value()
                            .unwrap(),
                    }
                };
                dispatch_receiver.dispatch.read_string_value_value()
            });
            receiver.state.with_mut(|state| state.target_path = value_2)
        };
        return Ok(options.clone());
    }
    if value
        .dispatch
        .clone()
        .downcast_template_value_to_dict_value()
        .is_some()
    {
        return parse_java_script_build_option_dictionary({
            let downcast_value_2 = &value;
            crate::node_modules::tsumo::engine::src::template::values::dict::DictValue {
                identity: downcast_value_2.identity.clone(),
                dispatch: downcast_value_2
                    .dispatch
                    .clone()
                    .downcast_template_value_to_dict_value()
                    .unwrap(),
            }
        });
    }
    Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
        String::from("TSUMO_TEMPLATE_JAVASCRIPT_BUILD_OPTIONS_INVALID"),
        String::from("js.Build options must be a dictionary or target path string"),
        None,
        None,
        None,
    )))
}

pub fn call_resource_function(
    name: String,
    args: js_abi::JsArray<
        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
    >,
    context: crate::node_modules::tsumo::engine::src::template::functions::function_context::TemplateFunctionContext,
) -> rt::TsonicResult<
    Option<crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue>,
> {
    let scope: crate::node_modules::tsumo::engine::src::template::scope::RenderScope =
        context.state.with(|state| state.scope.clone());
    let env: crate::node_modules::tsumo::engine::src::template::environment::TemplateEnvironment =
        context.state.with(|state| state.environment.clone());
    let overrides: js_abi::JsMap<
        String,
        js_abi::JsArray<crate::node_modules::tsumo::engine::src::template::nodes::TemplateNode>,
    > = context.state.with(|state| state.overrides.clone());
    if name == "resources.get" && tsonic_rust_runtime::conversions::usize_to_i32(args.len())? >= 1 {
        let mgr: Option<
            crate::node_modules::tsumo::engine::src::resources::manager::ResourceManager,
        > = {
            let dispatch_receiver = env.clone();
            dispatch_receiver
                .dispatch
                .clone()
                .dispatch_template_environment_get_resource_manager()
        };
        if mgr.is_none() {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let path: String =
            crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                .with(|module_binding| module_binding.load())
                .call((match args.get_number(0.0).as_ref() {
                    Some(flow_value) => flow_value.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },))?;
        let res: Option<crate::node_modules::tsumo::engine::src::resources::models::Resource> =
            (match mgr.as_ref() {
    Some(flow_value_2) => flow_value_2.clone(),
    None => unreachable!("checked flow selected a missing optional value"),
}).get(path.clone())?;
        return Ok(
            if res.is_some() {
                Some({
                    let upcast_value =
                        crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue::new(
                            match mgr.as_ref() {
                                Some(flow_value_3) => flow_value_3.clone(),
                                None => {
                                    unreachable!("checked flow selected a missing optional value")
                                }
                            },
                            match res.as_ref() {
                                Some(flow_value_4) => flow_value_4.clone(),
                                None => {
                                    unreachable!("checked flow selected a missing optional value")
                                }
                            },
                        );
                    crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                        identity: upcast_value.identity.clone(),
                        dispatch: upcast_value.dispatch.clone(),
                    }
                })
            } else {
                Some(crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()))
            },
        );
    }
    if name == "resources.getmatch"
        && tsonic_rust_runtime::conversions::usize_to_i32(args.len())? >= 1
    {
        let mgr: Option<
            crate::node_modules::tsumo::engine::src::resources::manager::ResourceManager,
        > = {
            let dispatch_receiver_2 = env.clone();
            dispatch_receiver_2
                .dispatch
                .clone()
                .dispatch_template_environment_get_resource_manager()
        };
        if mgr.is_none() {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let pattern: String =
            crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                .with(|module_binding| module_binding.load())
                .call((match args.get_number(0.0).as_ref() {
                    Some(flow_value_5) => flow_value_5.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },))?;
        let res: Option<crate::node_modules::tsumo::engine::src::resources::models::Resource> =
            (match mgr.as_ref() {
    Some(flow_value_6) => flow_value_6.clone(),
    None => unreachable!("checked flow selected a missing optional value"),
}).get_match(pattern.clone())?;
        return Ok(
            if res.is_some() {
                Some({
                    let upcast_value_2 =
                        crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue::new(
                            match mgr.as_ref() {
                                Some(flow_value_7) => flow_value_7.clone(),
                                None => {
                                    unreachable!("checked flow selected a missing optional value")
                                }
                            },
                            match res.as_ref() {
                                Some(flow_value_8) => flow_value_8.clone(),
                                None => {
                                    unreachable!("checked flow selected a missing optional value")
                                }
                            },
                        );
                    crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                        identity: upcast_value_2.identity.clone(),
                        dispatch: upcast_value_2.dispatch.clone(),
                    }
                })
            } else {
                Some(crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()))
            },
        );
    }
    if name == "resources.match" && tsonic_rust_runtime::conversions::usize_to_i32(args.len())? >= 1
    {
        let mgr: Option<
            crate::node_modules::tsumo::engine::src::resources::manager::ResourceManager,
        > = {
            let dispatch_receiver_3 = env.clone();
            dispatch_receiver_3
                .dispatch
                .clone()
                .dispatch_template_environment_get_resource_manager()
        };
        if mgr.is_none() {
            let empty_items: js_abi::JsArray<
                crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
            > = js_abi::JsArray::from_dense(vec![]);
            return Ok(
                Some({
                    let upcast_value_3 =
                        crate::node_modules::tsumo::engine::src::template::values::arrays::AnyArrayValue::new(
                            empty_items.clone(),
                        );
                    crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                        identity: upcast_value_3.identity.clone(),
                        dispatch: upcast_value_3.dispatch.clone(),
                    }
                }),
            );
        }
        let pattern: String =
            crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                .with(|module_binding| module_binding.load())
                .call((match args.get_number(0.0).as_ref() {
                    Some(flow_value_9) => flow_value_9.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },))?;
        let resources: js_abi::JsArray<
            crate::node_modules::tsumo::engine::src::resources::models::Resource,
        > = (match mgr.as_ref() {
    Some(flow_value_10) => flow_value_10.clone(),
    None => unreachable!("checked flow selected a missing optional value"),
}).r#match(pattern.clone())?;
        let result: js_abi::JsArray<
            crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
        > = js_abi::JsArray::from_dense(vec![]);
        {
            let mut i: f64 = 0.0;
            while i < (tsonic_rust_runtime::conversions::usize_to_i32(resources.len())? as f64) {
                tsonic_rust_runtime::conversions::usize_to_i32(
                    result.push_many([{
                        let upcast_value_4 =
                            crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue::new(
                                match mgr.as_ref() {
                                    Some(flow_value_11) => flow_value_11.clone(),
                                    None => {
                                        unreachable!(
                                            "checked flow selected a missing optional value"
                                        )
                                    }
                                },
                                match resources.get_number(i).as_ref() {
                                    Some(flow_value_12) => flow_value_12.clone(),
                                    None => {
                                        unreachable!(
                                            "checked flow selected a missing optional value"
                                        )
                                    }
                                },
                            );
                        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                            identity: upcast_value_4.identity.clone(),
                            dispatch: upcast_value_4.dispatch.clone(),
                        }
                    }]),
                )?;
                i += 1.0;
            }
        }
        return Ok(
            Some({
                let upcast_value_5 =
                    crate::node_modules::tsumo::engine::src::template::values::arrays::AnyArrayValue::new(
                        result.clone(),
                    );
                crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                    identity: upcast_value_5.identity.clone(),
                    dispatch: upcast_value_5.dispatch.clone(),
                }
            }),
        );
    }
    if name == "resources.bytype"
        && tsonic_rust_runtime::conversions::usize_to_i32(args.len())? >= 1
    {
        let mgr: Option<
            crate::node_modules::tsumo::engine::src::resources::manager::ResourceManager,
        > = {
            let dispatch_receiver_4 = env.clone();
            dispatch_receiver_4
                .dispatch
                .clone()
                .dispatch_template_environment_get_resource_manager()
        };
        if mgr.is_none() {
            let empty_items: js_abi::JsArray<
                crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
            > = js_abi::JsArray::from_dense(vec![]);
            return Ok(
                Some({
                    let upcast_value_6 =
                        crate::node_modules::tsumo::engine::src::template::values::arrays::AnyArrayValue::new(
                            empty_items.clone(),
                        );
                    crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                        identity: upcast_value_6.identity.clone(),
                        dispatch: upcast_value_6.dispatch.clone(),
                    }
                }),
            );
        }
        let media_type: String =
            crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                .with(|module_binding| module_binding.load())
                .call((match args.get_number(0.0).as_ref() {
                    Some(flow_value_13) => flow_value_13.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },))?;
        let resources: js_abi::JsArray<
            crate::node_modules::tsumo::engine::src::resources::models::Resource,
        > = (match mgr.as_ref() {
    Some(flow_value_14) => flow_value_14.clone(),
    None => unreachable!("checked flow selected a missing optional value"),
}).by_type(media_type.clone())?;
        let result: js_abi::JsArray<
            crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
        > = js_abi::JsArray::from_dense(vec![]);
        {
            let mut i: f64 = 0.0;
            while i < (tsonic_rust_runtime::conversions::usize_to_i32(resources.len())? as f64) {
                tsonic_rust_runtime::conversions::usize_to_i32(
                    result.push_many([{
                        let upcast_value_7 =
                            crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue::new(
                                match mgr.as_ref() {
                                    Some(flow_value_15) => flow_value_15.clone(),
                                    None => {
                                        unreachable!(
                                            "checked flow selected a missing optional value"
                                        )
                                    }
                                },
                                match resources.get_number(i).as_ref() {
                                    Some(flow_value_16) => flow_value_16.clone(),
                                    None => {
                                        unreachable!(
                                            "checked flow selected a missing optional value"
                                        )
                                    }
                                },
                            );
                        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                            identity: upcast_value_7.identity.clone(),
                            dispatch: upcast_value_7.dispatch.clone(),
                        }
                    }]),
                )?;
                i += 1.0;
            }
        }
        return Ok(
            Some({
                let upcast_value_8 =
                    crate::node_modules::tsumo::engine::src::template::values::arrays::AnyArrayValue::new(
                        result.clone(),
                    );
                crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                    identity: upcast_value_8.identity.clone(),
                    dispatch: upcast_value_8.dispatch.clone(),
                }
            }),
        );
    }
    if name == "resources.concat"
        && tsonic_rust_runtime::conversions::usize_to_i32(args.len())? >= 2
    {
        let mgr: Option<
            crate::node_modules::tsumo::engine::src::resources::manager::ResourceManager,
        > = {
            let dispatch_receiver_5 = env.clone();
            dispatch_receiver_5
                .dispatch
                .clone()
                .dispatch_template_environment_get_resource_manager()
        };
        if mgr.is_none() {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let target_path: String =
            crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                .with(|module_binding| module_binding.load())
                .call((match args.get_number(0.0).as_ref() {
                    Some(flow_value_17) => flow_value_17.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },))?;
        let input: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue =
            match args
                .get_number(tsonic_rust_runtime::conversions::i32_to_f64(
                    tsonic_rust_runtime::conversions::usize_to_i32(args.len())? - 1,
                ))
                .as_ref()
            {
                Some(flow_value_18) => flow_value_18.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            };
        let resources: js_abi::JsArray<
            crate::node_modules::tsumo::engine::src::resources::models::Resource,
        > = js_abi::JsArray::from_dense(vec![]);
        if input
            .dispatch
            .clone()
            .downcast_template_value_to_any_array_value()
            .is_some()
        {
            let arr: js_abi::JsArray<
                crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
            > =
                {
                    let dispatch_receiver_6 = &{
                        let downcast_value = &input;
                        crate::node_modules::tsumo::engine::src::template::values::arrays::AnyArrayValue {
                            identity: downcast_value.identity.clone(),
                            dispatch: downcast_value
                                .dispatch
                                .clone()
                                .downcast_template_value_to_any_array_value()
                                .unwrap(),
                        }
                    };
                    dispatch_receiver_6.dispatch.read_any_array_value_value()
                };
            {
                let mut i: f64 = 0.0;
                while i < (tsonic_rust_runtime::conversions::usize_to_i32(arr.len())? as f64) {
                    let item: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue =
                        match arr.get_number(i).as_ref() {
                            Some(flow_value_19) => flow_value_19.clone(),
                            None => unreachable!("checked flow selected a missing optional value"),
                        };
                    if item
                        .dispatch
                        .clone()
                        .downcast_template_value_to_resource_value()
                        .is_some()
                    {
                        tsonic_rust_runtime::conversions::usize_to_i32(
                            resources.push_many([{
                                let dispatch_receiver_7 = &{
                                    let downcast_value_2 = &item;
                                    crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue {
                                        identity: downcast_value_2.identity.clone(),
                                        dispatch: downcast_value_2
                                            .dispatch
                                            .clone()
                                            .downcast_template_value_to_resource_value()
                                            .unwrap(),
                                    }
                                };
                                dispatch_receiver_7.dispatch.read_resource_value_value()
                            }]),
                        )?;
                    }
                    i += 1.0;
                }
            }
        } else {
            if input
                .dispatch
                .clone()
                .downcast_template_value_to_resource_value()
                .is_some()
            {
                tsonic_rust_runtime::conversions::usize_to_i32(
                    resources.push_many([{
                        let dispatch_receiver_8 = &{
                            let downcast_value_3 = &input;
                            crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue {
                                identity: downcast_value_3.identity.clone(),
                                dispatch: downcast_value_3
                                    .dispatch
                                    .clone()
                                    .downcast_template_value_to_resource_value()
                                    .unwrap(),
                            }
                        };
                        dispatch_receiver_8.dispatch.read_resource_value_value()
                    }]),
                )?;
            }
        }
        if tsonic_rust_runtime::conversions::usize_to_i32(resources.len())? == 0 {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let res: crate::node_modules::tsumo::engine::src::resources::models::Resource = match mgr
            .as_ref()
        {
            Some(flow_value_20) => flow_value_20.clone(),
            None => unreachable!("checked flow selected a missing optional value"),
        }
        .concat(target_path.clone(), resources.clone())?;
        return Ok(
            Some({
                let upcast_value_9 =
                    crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue::new(
                        match mgr.as_ref() {
                            Some(flow_value_21) => flow_value_21.clone(),
                            None => unreachable!("checked flow selected a missing optional value"),
                        },
                        res.clone(),
                    );
                crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                    identity: upcast_value_9.identity.clone(),
                    dispatch: upcast_value_9.dispatch.clone(),
                }
            }),
        );
    }
    if name == "resources.fromstring"
        && tsonic_rust_runtime::conversions::usize_to_i32(args.len())? >= 2
    {
        let mgr: Option<
            crate::node_modules::tsumo::engine::src::resources::manager::ResourceManager,
        > = {
            let dispatch_receiver_9 = env.clone();
            dispatch_receiver_9
                .dispatch
                .clone()
                .dispatch_template_environment_get_resource_manager()
        };
        if mgr.is_none() {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let name_arg: String =
            crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                .with(|module_binding| module_binding.load())
                .call((match args.get_number(0.0).as_ref() {
                    Some(flow_value_22) => flow_value_22.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },))?;
        let content: String =
            crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                .with(|module_binding| module_binding.load())
                .call((match args.get_number(1.0).as_ref() {
                    Some(flow_value_23) => flow_value_23.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },))?;
        let res: crate::node_modules::tsumo::engine::src::resources::models::Resource = match mgr
            .as_ref()
        {
            Some(flow_value_24) => flow_value_24.clone(),
            None => unreachable!("checked flow selected a missing optional value"),
        }
        .from_string(name_arg.clone(), content.clone())?;
        return Ok(
            Some({
                let upcast_value_10 =
                    crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue::new(
                        match mgr.as_ref() {
                            Some(flow_value_25) => flow_value_25.clone(),
                            None => unreachable!("checked flow selected a missing optional value"),
                        },
                        res.clone(),
                    );
                crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                    identity: upcast_value_10.identity.clone(),
                    dispatch: upcast_value_10.dispatch.clone(),
                }
            }),
        );
    }
    if name == "resources.executeastemplate"
        && tsonic_rust_runtime::conversions::usize_to_i32(args.len())? >= 2
    {
        let mgr: Option<
            crate::node_modules::tsumo::engine::src::resources::manager::ResourceManager,
        > = {
            let dispatch_receiver_10 = env.clone();
            dispatch_receiver_10
                .dispatch
                .clone()
                .dispatch_template_environment_get_resource_manager()
        };
        if mgr.is_none() {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let piped: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue =
            if tsonic_rust_runtime::conversions::usize_to_i32(args.len())? >= 3 {
                match args
                    .get_number(tsonic_rust_runtime::conversions::i32_to_f64(
                        tsonic_rust_runtime::conversions::usize_to_i32(args.len())? - 1,
                    ))
                    .as_ref()
                {
                    Some(flow_value_26) => flow_value_26.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                }
            } else {
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load())
            };
        let is_resource: bool = piped
            .dispatch
            .clone()
            .downcast_template_value_to_resource_value()
            .is_some();
        if !is_resource {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let src: crate::node_modules::tsumo::engine::src::resources::models::Resource = {
            let dispatch_receiver_11 = &{
                let downcast_value_4 = &piped;
                crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue {
                    identity: downcast_value_4.identity.clone(),
                    dispatch: downcast_value_4
                        .dispatch
                        .clone()
                        .downcast_template_value_to_resource_value()
                        .unwrap(),
                }
            };
            dispatch_receiver_11.dispatch.read_resource_value_value()
        };
        let target_name: String =
            crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                .with(|module_binding| module_binding.load())
                .call((match args.get_number(0.0).as_ref() {
                    Some(flow_value_27) => flow_value_27.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },))?;
        let ctx: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue =
            match args.get_number(1.0).as_ref() {
                Some(flow_value_28) => flow_value_28.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            };
        let template_text: String =
            crate::node_modules::tsumo::engine::src::resources::text::read_resource_text(
                src.clone(),
                String::from("resources.ExecuteAsTemplate"),
            )?;
        let rendered: String = {
            let dispatch_receiver_12 = env.clone();
            dispatch_receiver_12
                .dispatch
                .clone()
                .dispatch_template_environment_render_text_template_source(
                    template_text.clone(),
                    ctx.clone(),
                    scope.state.with(|state| state.site.clone()),
                    overrides.clone(),
                    None,
                )
        }?;
        let bytes: tsonic_rust_node::buffer::Buffer =
            tsonic_rust_node::buffer::Buffer::from_string_enc(&rendered, "utf8")
                .map_err(tsonic_rust_runtime::TsonicError::from)?;
        let lang: String = scope
            .state
            .with(|state| state.site.clone())
            .state
            .with(|state| state.language.clone())
            .state
            .with(|state| state.lang.clone());
        let id: String = format!(
            "{}{}{}{}{}{}{}",
            String::from(""),
            rt::source_string(&src.state.with(|state| state.id.clone())),
            String::from("|executeAsTemplate:"),
            rt::source_string(&target_name),
            String::from("|lang:"),
            rt::source_string(&lang),
            String::from(""),
        );
        let out: crate::node_modules::tsumo::engine::src::resources::models::Resource =
            crate::node_modules::tsumo::engine::src::resources::models::Resource::new(
                id.clone(),
                src.state.with(|state| state.source_path.clone()),
                src.state.with(|state| state.publishable),
                Some(target_name.clone()),
                bytes.clone(),
                Some(rendered.clone()),
                crate::node_modules::tsumo::engine::src::resources::models::ResourceData::new(
                    String::from(""),
                ),
                None,
                None,
                None,
            );
        return Ok(
            Some({
                let upcast_value_11 =
                    crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue::new(
                        match mgr.as_ref() {
                            Some(flow_value_29) => flow_value_29.clone(),
                            None => unreachable!("checked flow selected a missing optional value"),
                        },
                        out.clone(),
                    );
                crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                    identity: upcast_value_11.identity.clone(),
                    dispatch: upcast_value_11.dispatch.clone(),
                }
            }),
        );
    }
    if name == "resources.minify" || name == "minify" {
        let mgr: Option<
            crate::node_modules::tsumo::engine::src::resources::manager::ResourceManager,
        > = {
            let dispatch_receiver_13 = env.clone();
            dispatch_receiver_13
                .dispatch
                .clone()
                .dispatch_template_environment_get_resource_manager()
        };
        if mgr.is_none() {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let piped: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue =
            if tsonic_rust_runtime::conversions::usize_to_i32(args.len())? >= 1 {
                match args
                    .get_number(tsonic_rust_runtime::conversions::i32_to_f64(
                        tsonic_rust_runtime::conversions::usize_to_i32(args.len())? - 1,
                    ))
                    .as_ref()
                {
                    Some(flow_value_30) => flow_value_30.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                }
            } else {
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load())
            };
        let is_resource: bool = piped
            .dispatch
            .clone()
            .downcast_template_value_to_resource_value()
            .is_some();
        if !is_resource {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let src: crate::node_modules::tsumo::engine::src::resources::models::Resource = {
            let dispatch_receiver_14 = &{
                let downcast_value_5 = &piped;
                crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue {
                    identity: downcast_value_5.identity.clone(),
                    dispatch: downcast_value_5
                        .dispatch
                        .clone()
                        .downcast_template_value_to_resource_value()
                        .unwrap(),
                }
            };
            dispatch_receiver_14.dispatch.read_resource_value_value()
        };
        let res: crate::node_modules::tsumo::engine::src::resources::models::Resource = match mgr
            .as_ref()
        {
            Some(flow_value_31) => flow_value_31.clone(),
            None => unreachable!("checked flow selected a missing optional value"),
        }
        .minify(src.clone())?;
        return Ok(
            Some({
                let upcast_value_12 =
                    crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue::new(
                        match mgr.as_ref() {
                            Some(flow_value_32) => flow_value_32.clone(),
                            None => unreachable!("checked flow selected a missing optional value"),
                        },
                        res.clone(),
                    );
                crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                    identity: upcast_value_12.identity.clone(),
                    dispatch: upcast_value_12.dispatch.clone(),
                }
            }),
        );
    }
    if (name == "resources.fingerprint" || name == "fingerprint")
        && tsonic_rust_runtime::conversions::usize_to_i32(args.len())? >= 1
    {
        let mgr: Option<
            crate::node_modules::tsumo::engine::src::resources::manager::ResourceManager,
        > = {
            let dispatch_receiver_15 = env.clone();
            dispatch_receiver_15
                .dispatch
                .clone()
                .dispatch_template_environment_get_resource_manager()
        };
        if mgr.is_none() {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let piped: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue =
            match args
                .get_number(tsonic_rust_runtime::conversions::i32_to_f64(
                    tsonic_rust_runtime::conversions::usize_to_i32(args.len())? - 1,
                ))
                .as_ref()
            {
                Some(flow_value_33) => flow_value_33.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            };
        let is_resource: bool = piped
            .dispatch
            .clone()
            .downcast_template_value_to_resource_value()
            .is_some();
        if !is_resource {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let src: crate::node_modules::tsumo::engine::src::resources::models::Resource = {
            let dispatch_receiver_16 = &{
                let downcast_value_6 = &piped;
                crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue {
                    identity: downcast_value_6.identity.clone(),
                    dispatch: downcast_value_6
                        .dispatch
                        .clone()
                        .downcast_template_value_to_resource_value()
                        .unwrap(),
                }
            };
            dispatch_receiver_16.dispatch.read_resource_value_value()
        };
        let res: crate::node_modules::tsumo::engine::src::resources::models::Resource = match mgr
            .as_ref()
        {
            Some(flow_value_34) => flow_value_34.clone(),
            None => unreachable!("checked flow selected a missing optional value"),
        }
        .fingerprint(src.clone())?;
        return Ok(
            Some({
                let upcast_value_13 =
                    crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue::new(
                        match mgr.as_ref() {
                            Some(flow_value_35) => flow_value_35.clone(),
                            None => unreachable!("checked flow selected a missing optional value"),
                        },
                        res.clone(),
                    );
                crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                    identity: upcast_value_13.identity.clone(),
                    dispatch: upcast_value_13.dispatch.clone(),
                }
            }),
        );
    }
    if name == "resources.copy" && tsonic_rust_runtime::conversions::usize_to_i32(args.len())? >= 2
    {
        let mgr: Option<
            crate::node_modules::tsumo::engine::src::resources::manager::ResourceManager,
        > = {
            let dispatch_receiver_17 = env.clone();
            dispatch_receiver_17
                .dispatch
                .clone()
                .dispatch_template_environment_get_resource_manager()
        };
        if mgr.is_none() {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let target_path: String =
            crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                .with(|module_binding| module_binding.load())
                .call((match args.get_number(0.0).as_ref() {
                    Some(flow_value_36) => flow_value_36.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },))?;
        let piped: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue =
            match args
                .get_number(tsonic_rust_runtime::conversions::i32_to_f64(
                    tsonic_rust_runtime::conversions::usize_to_i32(args.len())? - 1,
                ))
                .as_ref()
            {
                Some(flow_value_37) => flow_value_37.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            };
        let is_resource: bool = piped
            .dispatch
            .clone()
            .downcast_template_value_to_resource_value()
            .is_some();
        if !is_resource {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let src: crate::node_modules::tsumo::engine::src::resources::models::Resource = {
            let dispatch_receiver_18 = &{
                let downcast_value_7 = &piped;
                crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue {
                    identity: downcast_value_7.identity.clone(),
                    dispatch: downcast_value_7
                        .dispatch
                        .clone()
                        .downcast_template_value_to_resource_value()
                        .unwrap(),
                }
            };
            dispatch_receiver_18.dispatch.read_resource_value_value()
        };
        let res: crate::node_modules::tsumo::engine::src::resources::models::Resource = match mgr
            .as_ref()
        {
            Some(flow_value_38) => flow_value_38.clone(),
            None => unreachable!("checked flow selected a missing optional value"),
        }
        .copy(target_path.clone(), src.clone())?;
        return Ok(
            Some({
                let upcast_value_14 =
                    crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue::new(
                        match mgr.as_ref() {
                            Some(flow_value_39) => flow_value_39.clone(),
                            None => unreachable!("checked flow selected a missing optional value"),
                        },
                        res.clone(),
                    );
                crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                    identity: upcast_value_14.identity.clone(),
                    dispatch: upcast_value_14.dispatch.clone(),
                }
            }),
        );
    }
    if (name == "images.resize" || name == "resize")
        && tsonic_rust_runtime::conversions::usize_to_i32(args.len())? >= 1
    {
        let mgr: Option<
            crate::node_modules::tsumo::engine::src::resources::manager::ResourceManager,
        > = {
            let dispatch_receiver_19 = env.clone();
            dispatch_receiver_19
                .dispatch
                .clone()
                .dispatch_template_environment_get_resource_manager()
        };
        if mgr.is_none() {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let spec: String = if tsonic_rust_runtime::conversions::usize_to_i32(args.len())? >= 2 {
            crate::node_modules::tsumo::engine::src::template::runtime_helpers::TO_PLAIN_STRING
                .with(|module_binding| module_binding.load())
                .call((match args.get_number(0.0).as_ref() {
                    Some(flow_value_40) => flow_value_40.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },))?
        } else {
            String::from("")
        };
        let piped: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue =
            match args
                .get_number(tsonic_rust_runtime::conversions::i32_to_f64(
                    tsonic_rust_runtime::conversions::usize_to_i32(args.len())? - 1,
                ))
                .as_ref()
            {
                Some(flow_value_41) => flow_value_41.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            };
        let is_resource: bool = piped
            .dispatch
            .clone()
            .downcast_template_value_to_resource_value()
            .is_some();
        if !is_resource {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let src: crate::node_modules::tsumo::engine::src::resources::models::Resource = {
            let dispatch_receiver_20 = &{
                let downcast_value_8 = &piped;
                crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue {
                    identity: downcast_value_8.identity.clone(),
                    dispatch: downcast_value_8
                        .dispatch
                        .clone()
                        .downcast_template_value_to_resource_value()
                        .unwrap(),
                }
            };
            dispatch_receiver_20.dispatch.read_resource_value_value()
        };
        let res: crate::node_modules::tsumo::engine::src::resources::models::Resource = match mgr
            .as_ref()
        {
            Some(flow_value_42) => flow_value_42.clone(),
            None => unreachable!("checked flow selected a missing optional value"),
        }
        .resize(src.clone(), spec.clone())?;
        return Ok(
            Some({
                let upcast_value_15 =
                    crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue::new(
                        match mgr.as_ref() {
                            Some(flow_value_43) => flow_value_43.clone(),
                            None => unreachable!("checked flow selected a missing optional value"),
                        },
                        res.clone(),
                    );
                crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                    identity: upcast_value_15.identity.clone(),
                    dispatch: upcast_value_15.dispatch.clone(),
                }
            }),
        );
    }
    if name == "css.sass" && tsonic_rust_runtime::conversions::usize_to_i32(args.len())? >= 1 {
        let mgr: Option<
            crate::node_modules::tsumo::engine::src::resources::manager::ResourceManager,
        > = {
            let dispatch_receiver_21 = env.clone();
            dispatch_receiver_21
                .dispatch
                .clone()
                .dispatch_template_environment_get_resource_manager()
        };
        if mgr.is_none() {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let piped: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue =
            match args
                .get_number(tsonic_rust_runtime::conversions::i32_to_f64(
                    tsonic_rust_runtime::conversions::usize_to_i32(args.len())? - 1,
                ))
                .as_ref()
            {
                Some(flow_value_44) => flow_value_44.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            };
        let is_resource: bool = piped
            .dispatch
            .clone()
            .downcast_template_value_to_resource_value()
            .is_some();
        if !is_resource {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let src: crate::node_modules::tsumo::engine::src::resources::models::Resource = {
            let dispatch_receiver_22 = &{
                let downcast_value_9 = &piped;
                crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue {
                    identity: downcast_value_9.identity.clone(),
                    dispatch: downcast_value_9
                        .dispatch
                        .clone()
                        .downcast_template_value_to_resource_value()
                        .unwrap(),
                }
            };
            dispatch_receiver_22.dispatch.read_resource_value_value()
        };
        let res: crate::node_modules::tsumo::engine::src::resources::models::Resource = match mgr
            .as_ref()
        {
            Some(flow_value_45) => flow_value_45.clone(),
            None => unreachable!("checked flow selected a missing optional value"),
        }
        .sass_compile(src.clone())?;
        return Ok(
            Some({
                let upcast_value_16 =
                    crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue::new(
                        match mgr.as_ref() {
                            Some(flow_value_46) => flow_value_46.clone(),
                            None => unreachable!("checked flow selected a missing optional value"),
                        },
                        res.clone(),
                    );
                crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                    identity: upcast_value_16.identity.clone(),
                    dispatch: upcast_value_16.dispatch.clone(),
                }
            }),
        );
    }
    if name == "css.build" && tsonic_rust_runtime::conversions::usize_to_i32(args.len())? >= 1 {
        let mgr: Option<
            crate::node_modules::tsumo::engine::src::resources::manager::ResourceManager,
        > = {
            let dispatch_receiver_23 = env.clone();
            dispatch_receiver_23
                .dispatch
                .clone()
                .dispatch_template_environment_get_resource_manager()
        };
        if mgr.is_none() {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let piped: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue =
            match args
                .get_number(tsonic_rust_runtime::conversions::i32_to_f64(
                    tsonic_rust_runtime::conversions::usize_to_i32(args.len())? - 1,
                ))
                .as_ref()
            {
                Some(flow_value_47) => flow_value_47.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            };
        if piped
            .dispatch
            .clone()
            .downcast_template_value_to_resource_value()
            .is_some()
        {
            if tsonic_rust_runtime::conversions::usize_to_i32(args.len())? < 2 {
                return Ok(
                    Some({
                        let upcast_value_17 = build_css_resource(
                            match mgr.as_ref() {
                                Some(flow_value_48) => flow_value_48.clone(),
                                None => {
                                    unreachable!("checked flow selected a missing optional value")
                                }
                            },
                            {
                                let dispatch_receiver_24 = &{
                                    let downcast_value_10 = &piped;
                                    crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue {
                                        identity: downcast_value_10.identity.clone(),
                                        dispatch: downcast_value_10
                                            .dispatch
                                            .clone()
                                            .downcast_template_value_to_resource_value()
                                            .unwrap(),
                                    }
                                };
                                dispatch_receiver_24.dispatch.read_resource_value_value()
                            },
                            crate::node_modules::tsumo::engine::src::template::values::dict::DictValue::new(
                                js_abi::JsMap::new(),
                            ),
                        )?;
                        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                            identity: upcast_value_17.identity.clone(),
                            dispatch: upcast_value_17.dispatch.clone(),
                        }
                    }),
                );
            }
            let options: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue =
                match args.get_number(0.0).as_ref() {
                    Some(flow_value_49) => flow_value_49.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                };
            if options
                .dispatch
                .clone()
                .downcast_template_value_to_dict_value()
                .is_some()
            {
                return Ok(
                    Some({
                        let upcast_value_18 = build_css_resource(
                            match mgr.as_ref() {
                                Some(flow_value_50) => flow_value_50.clone(),
                                None => {
                                    unreachable!("checked flow selected a missing optional value")
                                }
                            },
                            {
                                let dispatch_receiver_25 = &{
                                    let downcast_value_11 = &piped;
                                    crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue {
                                        identity: downcast_value_11.identity.clone(),
                                        dispatch: downcast_value_11
                                            .dispatch
                                            .clone()
                                            .downcast_template_value_to_resource_value()
                                            .unwrap(),
                                    }
                                };
                                dispatch_receiver_25.dispatch.read_resource_value_value()
                            },
                            {
                                let downcast_value_12 = &options;
                                crate::node_modules::tsumo::engine::src::template::values::dict::DictValue {
                                    identity: downcast_value_12.identity.clone(),
                                    dispatch: downcast_value_12
                                        .dispatch
                                        .clone()
                                        .downcast_template_value_to_dict_value()
                                        .unwrap(),
                                }
                            },
                        )?;
                        crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                            identity: upcast_value_18.identity.clone(),
                            dispatch: upcast_value_18.dispatch.clone(),
                        }
                    }),
                );
            }
            return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
                String::from("TSUMO_TEMPLATE_CSS_BUILD_OPTIONS_INVALID"),
                String::from("css.Build options must be a dictionary"),
                None,
                None,
                None,
            )));
        }
        return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
            String::from("TSUMO_TEMPLATE_CSS_BUILD_INPUT_INVALID"),
            String::from("css.Build requires a CSS resource input"),
            None,
            None,
            None,
        )));
    }
    if name == "js.build" && tsonic_rust_runtime::conversions::usize_to_i32(args.len())? >= 1 {
        let manager: Option<
            crate::node_modules::tsumo::engine::src::resources::manager::ResourceManager,
        > = {
            let dispatch_receiver_26 = env.clone();
            dispatch_receiver_26
                .dispatch
                .clone()
                .dispatch_template_environment_get_resource_manager()
        };
        if manager.is_none() {
            return Ok(Some(
                crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
                    .with(|module_binding| module_binding.load()),
            ));
        }
        let piped: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue =
            match args
                .get_number(tsonic_rust_runtime::conversions::i32_to_f64(
                    tsonic_rust_runtime::conversions::usize_to_i32(args.len())? - 1,
                ))
                .as_ref()
            {
                Some(flow_value_51) => flow_value_51.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            };
        if piped
            .dispatch
            .clone()
            .downcast_template_value_to_resource_value()
            .is_none()
        {
            return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
                String::from("TSUMO_TEMPLATE_JAVASCRIPT_BUILD_INPUT_INVALID"),
                String::from("js.Build requires a JavaScript resource input"),
                None,
                None,
                None,
            )));
        }
        if tsonic_rust_runtime::conversions::usize_to_i32(args.len())? > 2 {
            return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
                String::from("TSUMO_TEMPLATE_JAVASCRIPT_BUILD_OPTIONS_INVALID"),
                String::from("js.Build accepts at most one options argument"),
                None,
                None,
                None,
            )));
        }
        let options: crate::node_modules::tsumo::engine::src::resources::javascript_provider::JavaScriptBuildOptions =
            if tsonic_rust_runtime::conversions::usize_to_i32(args.len())? == 2 {
                parse_java_script_build_options(
                    match args.get_number(0.0).as_ref() {
                        Some(flow_value_52) => flow_value_52.clone(),
                        None => unreachable!("checked flow selected a missing optional value"),
                    },
                )?
            } else {
                crate::node_modules::tsumo::engine::src::resources::javascript_provider::JavaScriptBuildOptions::new()
            };
        return Ok(
            Some({
                let upcast_value_19 =
                    crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue::new(
                        match manager.as_ref() {
                            Some(flow_value_53) => flow_value_53.clone(),
                            None => unreachable!("checked flow selected a missing optional value"),
                        },
                        (match manager.as_ref() {
    Some(flow_value_54) => flow_value_54.clone(),
    None => unreachable!("checked flow selected a missing optional value"),
}).javascript_build(
                            {
                                let dispatch_receiver_27 = &{
                                    let downcast_value_13 = &piped;
                                    crate::node_modules::tsumo::engine::src::template::values::resources::ResourceValue {
                                        identity: downcast_value_13.identity.clone(),
                                        dispatch: downcast_value_13
                                            .dispatch
                                            .clone()
                                            .downcast_template_value_to_resource_value()
                                            .unwrap(),
                                    }
                                };
                                dispatch_receiver_27.dispatch.read_resource_value_value()
                            },
                            options.clone(),
                        )?,
                    );
                crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue {
                    identity: upcast_value_19.identity.clone(),
                    dispatch: upcast_value_19.dispatch.clone(),
                }
            }),
        );
    }
    Ok(
        Option::<crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue>::None,
    )
}
