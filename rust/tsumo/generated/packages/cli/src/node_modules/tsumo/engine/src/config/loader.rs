// Generated by the Tsonic Rust target. Do not edit.

use tsonic_rust_js::abi as js_abi;

use tsonic_rust_js::string as js_string;

use crate::program as rt;

pub(crate) fn load_split_config(
    config_dir: String,
) -> rt::TsonicResult<crate::node_modules::tsumo::engine::src::models::site_config::SiteConfig> {
    crate::node_modules::tsumo::engine::src::fs::REJECT_FILESYSTEM_LINK
        .with(|module_binding| module_binding.load())
        .call((config_dir.clone(),))?;
    let mut config: crate::node_modules::tsumo::engine::src::models::site_config::SiteConfig =
        crate::node_modules::tsumo::engine::src::models::site_config::SiteConfig::new(
            String::from("Tsumo Site"),
            String::from(""),
            String::from("en-us"),
            Option::<String>::None,
            Option::<String>::None,
        );
    let entries: js_abi::JsArray<String> = tsonic_rust_node::fs::readdir_sync(&config_dir)
        .map_err(tsonic_rust_runtime::TsonicError::from)?;
    let files: js_abi::JsArray<String> = js_abi::JsArray::from_dense(vec![]);
    {
        let mut i: f64 = 0.0;
        while i < (tsonic_rust_runtime::conversions::usize_to_i32(entries.len())? as f64) {
            let path: String = tsonic_rust_node::path::join(&[
                config_dir.as_str(),
                (match entries.get_number(i).as_ref() {
    Some(flow_value) => flow_value.clone(),
    None => unreachable!("checked flow selected a missing optional value"),
}).as_str(),
            ]);
            crate::node_modules::tsumo::engine::src::fs::REJECT_FILESYSTEM_LINK
                .with(|module_binding| module_binding.load())
                .call((path.clone(),))?;
            if !tsonic_rust_node::fs::stat_sync(&path)
                .map_err(tsonic_rust_runtime::TsonicError::from)?
                .is_file()
            {
                return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
                    String::from("TSUMO_CONFIG_ENTRY_INVALID"),
                    format!(
                        "{}{}{}",
                        String::from("Split configuration entry is not a file: "),
                        rt::source_string(&path),
                        String::from(""),
                    ),
                    Some(path.clone()),
                    None,
                    None,
                )));
            }
            tsonic_rust_runtime::conversions::usize_to_i32(files.push_many([path.clone()]))?;
            i += 1.0;
        }
    }
    let sorted_files: js_abi::JsArray<String> = js_abi::JsArray::from_dense(vec![]);
    let base_files: js_abi::JsArray<String> = js_abi::JsArray::from_dense(vec![]);
    let param_files: js_abi::JsArray<String> = js_abi::JsArray::from_dense(vec![]);
    let lang_files: js_abi::JsArray<String> = js_abi::JsArray::from_dense(vec![]);
    let menu_files: js_abi::JsArray<String> = js_abi::JsArray::from_dense(vec![]);
    let module_files: js_abi::JsArray<String> = js_abi::JsArray::from_dense(vec![]);
    let other_files: js_abi::JsArray<String> = js_abi::JsArray::from_dense(vec![]);
    let file_names: js_abi::JsSet<String> = js_abi::JsSet::new();
    {
        let mut i: f64 = 0.0;
        while i < (tsonic_rust_runtime::conversions::usize_to_i32(files.len())? as f64) {
            let file_path: String = match files.get_number(i).as_ref() {
                Some(flow_value_2) => flow_value_2.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            };
            let name: String =
                js_string::to_lower_case(&tsonic_rust_node::path::basename(&file_path, None));
            if file_names.has(&name) {
                return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
                    String::from("TSUMO_CONFIG_FILE_AMBIGUOUS"),
                    format!(
                        "{}{}{}",
                        String::from("Split configuration file name '"),
                        rt::source_string(&name),
                        String::from("' is not unique"),
                    ),
                    Some(config_dir.clone()),
                    None,
                    None,
                )));
            }
            file_names.add(name.clone());
            if name == "hugo.toml"
                || name == "hugo.yaml"
                || name == "hugo.yml"
                || name == "config.toml"
                || name == "config.yaml"
                || name == "config.yml"
            {
                tsonic_rust_runtime::conversions::usize_to_i32(
                    base_files.push_many([file_path.clone()]),
                )?;
            } else {
                if name == "params.toml" || name == "params.yaml" || name == "params.yml" {
                    tsonic_rust_runtime::conversions::usize_to_i32(
                        param_files.push_many([file_path.clone()]),
                    )?;
                } else {
                    if js_string::starts_with_from_start(&name, "languages.") {
                        tsonic_rust_runtime::conversions::usize_to_i32(
                            lang_files.push_many([file_path.clone()]),
                        )?;
                    } else {
                        if js_string::starts_with_from_start(&name, "menus.") {
                            tsonic_rust_runtime::conversions::usize_to_i32(
                                menu_files.push_many([file_path.clone()]),
                            )?;
                        } else {
                            if name == "module.toml" {
                                tsonic_rust_runtime::conversions::usize_to_i32(
                                    module_files.push_many([file_path.clone()]),
                                )?;
                            } else {
                                tsonic_rust_runtime::conversions::usize_to_i32(
                                    other_files.push_many([file_path.clone()]),
                                )?;
                            }
                        }
                    }
                }
            }
            i += 1.0;
        }
    }
    base_files.sort_by_js_string();
    param_files.sort_by_js_string();
    lang_files.sort_by_js_string();
    menu_files.sort_by_js_string();
    module_files.sort_by_js_string();
    other_files.sort_by_js_string();
    if tsonic_rust_runtime::conversions::usize_to_i32(base_files.len())? > 1 {
        return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
            String::from("TSUMO_CONFIG_FILE_AMBIGUOUS"),
            String::from("Split configuration accepts at most one base configuration file"),
            Some(config_dir.clone()),
            None,
            None,
        )));
    }
    if tsonic_rust_runtime::conversions::usize_to_i32(param_files.len())? > 1 {
        return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
            String::from("TSUMO_CONFIG_FILE_AMBIGUOUS"),
            String::from("Split configuration accepts at most one params configuration file"),
            Some(config_dir.clone()),
            None,
            None,
        )));
    }
    if tsonic_rust_runtime::conversions::usize_to_i32(module_files.len())? > 1 {
        return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
            String::from("TSUMO_CONFIG_FILE_AMBIGUOUS"),
            String::from("Split configuration accepts at most one module configuration file"),
            Some(config_dir.clone()),
            None,
            None,
        )));
    }
    let mut aggregate_language_files: f64 = 0.0;
    {
        let mut i: f64 = 0.0;
        while i < (tsonic_rust_runtime::conversions::usize_to_i32(lang_files.len())? as f64) {
            if js_string::to_lower_case(&tsonic_rust_node::path::basename(
                &match lang_files.get_number(i).as_ref() {
                    Some(flow_value_3) => flow_value_3.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },
                None,
            )) == "languages.toml"
            {
                aggregate_language_files += 1.0;
            }
            i += 1.0;
        }
    }
    if aggregate_language_files > 1.0 {
        return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
            String::from("TSUMO_CONFIG_FILE_AMBIGUOUS"),
            String::from("Split configuration accepts at most one aggregate language configuration file"),
            Some(config_dir.clone()),
            None,
            None,
        )));
    }
    {
        let mut i: f64 = 0.0;
        while i < (tsonic_rust_runtime::conversions::usize_to_i32(base_files.len())? as f64) {
            tsonic_rust_runtime::conversions::usize_to_i32(sorted_files.push_many([match base_files
                .get_number(i)
                .as_ref()
            {
                Some(flow_value_4) => flow_value_4.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            }]))?;
            i += 1.0;
        }
    }
    {
        let mut i: f64 = 0.0;
        while i < (tsonic_rust_runtime::conversions::usize_to_i32(param_files.len())? as f64) {
            tsonic_rust_runtime::conversions::usize_to_i32(
                sorted_files.push_many([match param_files.get_number(i).as_ref() {
                    Some(flow_value_5) => flow_value_5.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                }]),
            )?;
            i += 1.0;
        }
    }
    {
        let mut i: f64 = 0.0;
        while i < (tsonic_rust_runtime::conversions::usize_to_i32(lang_files.len())? as f64) {
            if js_string::to_lower_case(&tsonic_rust_node::path::basename(
                &match lang_files.get_number(i).as_ref() {
                    Some(flow_value_6) => flow_value_6.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },
                None,
            )) == "languages.toml"
            {
                tsonic_rust_runtime::conversions::usize_to_i32(
                    sorted_files.push_many([match lang_files.get_number(i).as_ref() {
                        Some(flow_value_7) => flow_value_7.clone(),
                        None => unreachable!("checked flow selected a missing optional value"),
                    }]),
                )?;
            }
            i += 1.0;
        }
    }
    {
        let mut i: f64 = 0.0;
        while i < (tsonic_rust_runtime::conversions::usize_to_i32(lang_files.len())? as f64) {
            if js_string::to_lower_case(&tsonic_rust_node::path::basename(
                &match lang_files.get_number(i).as_ref() {
                    Some(flow_value_8) => flow_value_8.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                },
                None,
            )) != "languages.toml"
            {
                tsonic_rust_runtime::conversions::usize_to_i32(
                    sorted_files.push_many([match lang_files.get_number(i).as_ref() {
                        Some(flow_value_9) => flow_value_9.clone(),
                        None => unreachable!("checked flow selected a missing optional value"),
                    }]),
                )?;
            }
            i += 1.0;
        }
    }
    {
        let mut i: f64 = 0.0;
        while i < (tsonic_rust_runtime::conversions::usize_to_i32(menu_files.len())? as f64) {
            tsonic_rust_runtime::conversions::usize_to_i32(sorted_files.push_many([match menu_files
                .get_number(i)
                .as_ref()
            {
                Some(flow_value_10) => flow_value_10.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            }]))?;
            i += 1.0;
        }
    }
    {
        let mut i: f64 = 0.0;
        while i < (tsonic_rust_runtime::conversions::usize_to_i32(module_files.len())? as f64) {
            tsonic_rust_runtime::conversions::usize_to_i32(
                sorted_files.push_many([match module_files.get_number(i).as_ref() {
                    Some(flow_value_11) => flow_value_11.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                }]),
            )?;
            i += 1.0;
        }
    }
    {
        let mut i: f64 = 0.0;
        while i < (tsonic_rust_runtime::conversions::usize_to_i32(other_files.len())? as f64) {
            tsonic_rust_runtime::conversions::usize_to_i32(
                sorted_files.push_many([match other_files.get_number(i).as_ref() {
                    Some(flow_value_12) => flow_value_12.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                }]),
            )?;
            i += 1.0;
        }
    }
    {
        let mut i: f64 = 0.0;
        while i < (tsonic_rust_runtime::conversions::usize_to_i32(sorted_files.len())? as f64) {
            let file_path: String = match sorted_files.get_number(i).as_ref() {
                Some(flow_value_13) => flow_value_13.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            };
            let file_name: String =
                js_string::to_lower_case(&tsonic_rust_node::path::basename(&file_path, None));
            let text: String = crate::node_modules::tsumo::engine::src::fs::READ_TEXT_FILE
                .with(|module_binding| module_binding.load())
                .call((file_path.clone(),))?;
            if js_string::ends_with_at_end(&file_name, ".toml") {
                config =
                    crate::node_modules::tsumo::engine::src::config::toml::merge_toml_into_config(
                        config.clone(),
                        text.clone(),
                        file_name.clone(),
                        Some(file_path.clone()),
                    )?;
            } else {
                if js_string::ends_with_at_end(&file_name, ".yaml")
                    || js_string::ends_with_at_end(&file_name, ".yml")
                {
                    config =
                        crate::node_modules::tsumo::engine::src::config::yaml::merge_yaml_into_config(
                            config.clone(),
                            text.clone(),
                            file_name.clone(),
                            Some(file_path.clone()),
                        )?;
                } else {
                    return Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
                        String::from("TSUMO_CONFIG_FILE_UNSUPPORTED"),
                        format!(
                            "{}{}{}",
                            String::from("Unsupported split configuration file '"),
                            rt::source_string(&file_name),
                            String::from("'"),
                        ),
                        Some(file_path.clone()),
                        None,
                        None,
                    )));
                }
            }
            i += 1.0;
        }
    }
    Ok(config.clone())
}

pub fn load_site_config(
    site_dir: String,
) -> rt::TsonicResult<
    crate::node_modules::tsumo::engine::src::config::loaded_config::LoadedConfig,
> {
    let split_config_dir: String =
        tsonic_rust_node::path::join(&[site_dir.as_str(), "config", "_default"]);
    if crate::node_modules::tsumo::engine::src::fs::DIR_EXISTS
        .with(|module_binding| module_binding.load())
        .call((split_config_dir.clone(),))?
    {
        return Ok(crate::node_modules::tsumo::engine::src::config::loaded_config::LoadedConfig::new(
            Some(split_config_dir.clone()),
            load_split_config(split_config_dir.clone())?,
        ));
    }
    let candidates: js_abi::JsArray<String> = js_abi::JsArray::from_dense(vec![
        tsonic_rust_node::path::join(&[site_dir.as_str(), "hugo.toml"]),
        tsonic_rust_node::path::join(&[site_dir.as_str(), "hugo.yaml"]),
        tsonic_rust_node::path::join(&[site_dir.as_str(), "hugo.yml"]),
        tsonic_rust_node::path::join(&[site_dir.as_str(), "hugo.json"]),
        tsonic_rust_node::path::join(&[site_dir.as_str(), "config.toml"]),
        tsonic_rust_node::path::join(&[site_dir.as_str(), "config.yaml"]),
        tsonic_rust_node::path::join(&[site_dir.as_str(), "config.yml"]),
        tsonic_rust_node::path::join(&[site_dir.as_str(), "config.json"]),
    ]);
    let path: Option<String> =
        crate::node_modules::tsumo::engine::src::config::helpers::try_get_first_existing(
            candidates.clone(),
        )?;
    if path.is_none() {
        return Ok(crate::node_modules::tsumo::engine::src::config::loaded_config::LoadedConfig::new(
            Option::<String>::None,
            crate::node_modules::tsumo::engine::src::models::site_config::SiteConfig::new(
                String::from("Tsumo Site"),
                String::from(""),
                String::from("en-us"),
                Option::<String>::None,
                Option::<String>::None,
            ),
        ));
    }
    let text: String = crate::node_modules::tsumo::engine::src::fs::READ_TEXT_FILE
        .with(|module_binding| module_binding.load())
        .call((match path.as_ref() {
            Some(flow_value) => flow_value.clone(),
            None => unreachable!("checked flow selected a missing optional value"),
        },))?;
    let lower: String = js_string::to_lower_case(&match path.as_ref() {
        Some(flow_value_2) => flow_value_2.clone(),
        None => unreachable!("checked flow selected a missing optional value"),
    });
    let parsed_config: crate::node_modules::tsumo::engine::src::models::site_config::SiteConfig =
        if js_string::ends_with_at_end(&lower, ".toml") {
            crate::node_modules::tsumo::engine::src::config::toml::parse_toml_config(
                text.clone(),
                Some(match path.as_ref() {
                    Some(flow_value_3) => flow_value_3.clone(),
                    None => unreachable!("checked flow selected a missing optional value"),
                }),
            )?
        } else {
            if js_string::ends_with_at_end(&lower, ".json") {
                crate::node_modules::tsumo::engine::src::config::json::parse_json_config(
                    text.clone(),
                    Some(match path.as_ref() {
                        Some(flow_value_4) => flow_value_4.clone(),
                        None => unreachable!("checked flow selected a missing optional value"),
                    }),
                )?
            } else {
                crate::node_modules::tsumo::engine::src::config::yaml::parse_yaml_config(
                    &text,
                    Some(match path.as_ref() {
                        Some(flow_value_5) => flow_value_5.clone(),
                        None => unreachable!("checked flow selected a missing optional value"),
                    }),
                )?
            }
        };
    Ok(crate::node_modules::tsumo::engine::src::config::loaded_config::LoadedConfig::new(
        Some(match path.as_ref() {
            Some(flow_value_6) => flow_value_6.clone(),
            None => unreachable!("checked flow selected a missing optional value"),
        }),
        parsed_config.clone(),
    ))
}
