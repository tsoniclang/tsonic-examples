// Generated by the Tsonic Rust target. Do not edit.

use tsonic_rust_js::abi as js_abi;

use tsonic_rust_js::string as js_string;

use crate::program as rt;

#[allow(dead_code, reason = "preserves the checked source contract")]
pub(crate) struct TemplateEvaluationContextState {
    pub(crate) scope: crate::node_modules::tsumo::engine::src::template::scope::RenderScope,
    pub(crate) environment:
        crate::node_modules::tsumo::engine::src::template::environment::TemplateEnvironment,
    pub(crate) overrides:
        js_abi::JsMap<
            String,
            js_abi::JsArray<
                crate::node_modules::tsumo::engine::src::template::nodes::TemplateNode,
            >,
        >,
    pub(crate) defines:
        js_abi::JsMap<
            String,
            js_abi::JsArray<
                crate::node_modules::tsumo::engine::src::template::nodes::TemplateNode,
            >,
        >,
}

#[derive(Clone, Debug, PartialEq)]
pub struct TemplateEvaluationContext {
    pub(crate) state: rt::ObjectHandle<TemplateEvaluationContextState>,
}

impl TemplateEvaluationContext {
    pub fn new(
        scope: crate::node_modules::tsumo::engine::src::template::scope::RenderScope,
        environment: crate::node_modules::tsumo::engine::src::template::environment::TemplateEnvironment,
        overrides: js_abi::JsMap<
            String,
            js_abi::JsArray<
                crate::node_modules::tsumo::engine::src::template::nodes::TemplateNode,
            >,
        >,
        defines: js_abi::JsMap<
            String,
            js_abi::JsArray<
                crate::node_modules::tsumo::engine::src::template::nodes::TemplateNode,
            >,
        >,
    ) -> TemplateEvaluationContext {
        let field_scope: crate::node_modules::tsumo::engine::src::template::scope::RenderScope =
            scope.clone();
        let field_environment: crate::node_modules::tsumo::engine::src::template::environment::TemplateEnvironment =
            environment.clone();
        let field_overrides: js_abi::JsMap<
            String,
            js_abi::JsArray<
                crate::node_modules::tsumo::engine::src::template::nodes::TemplateNode,
            >,
        > = overrides.clone();
        let field_defines: js_abi::JsMap<
            String,
            js_abi::JsArray<
                crate::node_modules::tsumo::engine::src::template::nodes::TemplateNode,
            >,
        > = defines.clone();
        TemplateEvaluationContext {
            state: rt::ObjectHandle::new(TemplateEvaluationContextState {
                scope: field_scope,
                environment: field_environment,
                overrides: field_overrides,
                defines: field_defines,
            }),
        }
    }
}

pub fn evaluate_pipeline(
    pipeline: crate::node_modules::tsumo::engine::src::template::syntax::expressions::Pipeline,
    scope: crate::node_modules::tsumo::engine::src::template::scope::RenderScope,
    environment: crate::node_modules::tsumo::engine::src::template::environment::TemplateEnvironment,
    overrides: js_abi::JsMap<
        String,
        js_abi::JsArray<crate::node_modules::tsumo::engine::src::template::nodes::TemplateNode>,
    >,
    defines: js_abi::JsMap<
        String,
        js_abi::JsArray<crate::node_modules::tsumo::engine::src::template::nodes::TemplateNode>,
    >,
) -> rt::TsonicResult<
    crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
> {
    let context: TemplateEvaluationContext = TemplateEvaluationContext::new(
        scope.clone(),
        environment.clone(),
        overrides.clone(),
        defines.clone(),
    );
    if tsonic_rust_runtime::conversions::usize_to_i32(
        pipeline.state.with(|state| state.stages.clone()).len(),
    )? == 0
    {
        return Ok(crate::node_modules::tsumo::engine::src::template::runtime_helpers::NIL
            .with(|module_binding| module_binding.load()));
    }
    let mut value: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue =
        evaluate_command(
            match pipeline
                .state
                .with(|state| state.stages.clone())
                .get_number(0.0)
                .as_ref()
            {
                Some(flow_value) => flow_value.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            },
            context.clone(),
            Option::<crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue>::None,
        )?;
    {
        let mut index: f64 = 1.0;
        while index
            < (tsonic_rust_runtime::conversions::usize_to_i32(pipeline.state.with(|state| state.stages.clone()).len())? as f64)
        {
            value = evaluate_command(match pipeline
    .state
    .with(|state| state.stages.clone())
    .get_number(index)
    .as_ref()
{
    Some(flow_value_2) => flow_value_2.clone(),
    None => unreachable!("checked flow selected a missing optional value"),
}, context.clone(), Some(value.clone()))?;
            index += 1.0;
        }
    }
    Ok(value.clone())
}

pub(crate) fn evaluate_command(
    command: crate::node_modules::tsumo::engine::src::template::syntax::expressions::Command,
    context: TemplateEvaluationContext,
    piped: Option<crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue>,
) -> rt::TsonicResult<
    crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
> {
    if tsonic_rust_runtime::conversions::usize_to_i32(
        command.state.with(|state| state.args.clone()).len(),
    )? == 0
        && piped.is_none()
    {
        return evaluate_expression(
            command.state.with(|state| state.head.clone()),
            context.clone(),
        );
    }
    let head: crate::node_modules::tsumo::engine::src::template::syntax::expressions::Expr =
        command.state.with(|state| state.head.clone());
    if head
        .dispatch
        .clone()
        .downcast_expr_to_token_expr()
        .is_some()
    {
        let args: js_abi::JsArray<
            crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
        > = js_abi::JsArray::from_dense(vec![]);
        {
            let mut index: f64 = 0.0;
            while index
                < (tsonic_rust_runtime::conversions::usize_to_i32(command.state.with(|state| state.args.clone()).len())? as f64)
            {
                tsonic_rust_runtime::conversions::usize_to_i32(args.push_many([evaluate_expression(
                    match command
                        .state
                        .with(|state| state.args.clone())
                        .get_number(index)
                        .as_ref()
                    {
                        Some(flow_value) => flow_value.clone(),
                        None => unreachable!("checked flow selected a missing optional value"),
                    },
                    context.clone(),
                )?]))?;
                index += 1.0;
            }
        }
        if piped.is_some() {
            tsonic_rust_runtime::conversions::usize_to_i32(args.push_many([match piped.as_ref() {
                Some(flow_value_2) => flow_value_2.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            }]))?;
        }
        return crate::node_modules::tsumo::engine::src::template::functions::call_function::call_template_function(
            {
                let dispatch_receiver = &{
                    let downcast_value = &head;
                    crate::node_modules::tsumo::engine::src::template::syntax::expressions::TokenExpr {
                        identity: downcast_value.identity.clone(),
                        dispatch: downcast_value
                            .dispatch
                            .clone()
                            .downcast_expr_to_token_expr()
                            .unwrap(),
                    }
                };
                dispatch_receiver.dispatch.read_token_expr_token()
            },
            args.clone(),
            context.state.with(|state| state.scope.clone()),
            context.state.with(|state| state.environment.clone()),
            context.state.with(|state| state.overrides.clone()),
            context.state.with(|state| state.defines.clone()),
        );
    }
    if head
        .dispatch
        .clone()
        .downcast_expr_to_access_expr()
        .is_some()
        && tsonic_rust_runtime::conversions::usize_to_i32(
            {
                let dispatch_receiver_2 = &{
                    let downcast_value_2 = &head;
                    crate::node_modules::tsumo::engine::src::template::syntax::expressions::AccessExpr {
                        identity: downcast_value_2.identity.clone(),
                        dispatch: downcast_value_2
                            .dispatch
                            .clone()
                            .downcast_expr_to_access_expr()
                            .unwrap(),
                    }
                };
                dispatch_receiver_2.dispatch.read_access_expr_segments()
            }
            .len(),
        )?
            > 0
    {
        let mut receiver: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue =
            evaluate_expression(
                {
                    let dispatch_receiver_3 = &{
                        let downcast_value_3 = &head;
                        crate::node_modules::tsumo::engine::src::template::syntax::expressions::AccessExpr {
                            identity: downcast_value_3.identity.clone(),
                            dispatch: downcast_value_3
                                .dispatch
                                .clone()
                                .downcast_expr_to_access_expr()
                                .unwrap(),
                        }
                    };
                    dispatch_receiver_3.dispatch.read_access_expr_base()
                },
                context.clone(),
            )?;
        if tsonic_rust_runtime::conversions::usize_to_i32(
            {
                let dispatch_receiver_4 = &{
                    let downcast_value_4 = &head;
                    crate::node_modules::tsumo::engine::src::template::syntax::expressions::AccessExpr {
                        identity: downcast_value_4.identity.clone(),
                        dispatch: downcast_value_4
                            .dispatch
                            .clone()
                            .downcast_expr_to_access_expr()
                            .unwrap(),
                    }
                };
                dispatch_receiver_4.dispatch.read_access_expr_segments()
            }
            .len(),
        )?
            > 1
        {
            let receiver_segments: js_abi::JsArray<String> = js_abi::JsArray::from_dense(vec![]);
            {
                let mut index: f64 = 0.0;
                while index
                    < ((tsonic_rust_runtime::conversions::usize_to_i32({ let dispatch_receiver_5 = &{ let downcast_value_5 = &head; crate::node_modules::tsumo::engine::src::template::syntax::expressions::AccessExpr { identity: downcast_value_5.identity.clone(), dispatch: downcast_value_5.dispatch.clone().downcast_expr_to_access_expr().unwrap() } }; dispatch_receiver_5.dispatch.read_access_expr_segments() }.len())? - 1) as f64)
                {
                    tsonic_rust_runtime::conversions::usize_to_i32(
                        receiver_segments.push_many([match {
                            let dispatch_receiver_6 = &{
                                let downcast_value_6 = &head;
                                crate::node_modules::tsumo::engine::src::template::syntax::expressions::AccessExpr {
                                    identity: downcast_value_6.identity.clone(),
                                    dispatch: downcast_value_6
                                        .dispatch
                                        .clone()
                                        .downcast_expr_to_access_expr()
                                        .unwrap(),
                                }
                            };
                            dispatch_receiver_6.dispatch.read_access_expr_segments()
                        }
                        .get_number(index)
                        .as_ref()
                        {
                            Some(flow_value_3) => flow_value_3.clone(),
                            None => unreachable!("checked flow selected a missing optional value"),
                        }]),
                    )?;
                    index += 1.0;
                }
            }
            receiver =
                crate::node_modules::tsumo::engine::src::template::evaluation::property_semantics::resolve_path(
                    receiver.clone(),
                    receiver_segments.clone(),
                    context.state.with(|state| state.scope.clone()),
                )?;
        }
        let args: js_abi::JsArray<
            crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
        > = js_abi::JsArray::from_dense(vec![]);
        {
            let mut index: f64 = 0.0;
            while index
                < (tsonic_rust_runtime::conversions::usize_to_i32(command.state.with(|state| state.args.clone()).len())? as f64)
            {
                tsonic_rust_runtime::conversions::usize_to_i32(args.push_many([evaluate_expression(
                    match command
                        .state
                        .with(|state| state.args.clone())
                        .get_number(index)
                        .as_ref()
                    {
                        Some(flow_value_4) => flow_value_4.clone(),
                        None => unreachable!("checked flow selected a missing optional value"),
                    },
                    context.clone(),
                )?]))?;
                index += 1.0;
            }
        }
        if piped.is_some() {
            tsonic_rust_runtime::conversions::usize_to_i32(args.push_many([match piped.as_ref() {
                Some(flow_value_5) => flow_value_5.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            }]))?;
        }
        return crate::node_modules::tsumo::engine::src::template::evaluation::expression_semantics::call_method(
            receiver.clone(),
            match {
                let dispatch_receiver_7 = &{
                    let downcast_value_7 = &head;
                    crate::node_modules::tsumo::engine::src::template::syntax::expressions::AccessExpr {
                        identity: downcast_value_7.identity.clone(),
                        dispatch: downcast_value_7
                            .dispatch
                            .clone()
                            .downcast_expr_to_access_expr()
                            .unwrap(),
                    }
                };
                dispatch_receiver_7.dispatch.read_access_expr_segments()
            }
            .get_number(
                    tsonic_rust_runtime::conversions::i32_to_f64(
                        tsonic_rust_runtime::conversions::usize_to_i32(
                            {
                                let dispatch_receiver_8 = &{
                                    let downcast_value_8 = &head;
                                    crate::node_modules::tsumo::engine::src::template::syntax::expressions::AccessExpr {
                                        identity: downcast_value_8.identity.clone(),
                                        dispatch: downcast_value_8
                                            .dispatch
                                            .clone()
                                            .downcast_expr_to_access_expr()
                                            .unwrap(),
                                    }
                                };
                                dispatch_receiver_8.dispatch.read_access_expr_segments()
                            }
                            .len(),
                        )?
                            - 1,
                    ),
                )
            .as_ref()
            {
                Some(flow_value_6) => flow_value_6.clone(),
                None => unreachable!("checked flow selected a missing optional value"),
            },
            args.clone(),
            context.state.with(|state| state.scope.clone()),
            context.state.with(|state| state.environment.clone()),
            context.state.with(|state| state.overrides.clone()),
            context.state.with(|state| state.defines.clone()),
        );
    }
    if piped.is_some() {
        return Ok(match piped.as_ref() {
            Some(flow_value_7) => flow_value_7.clone(),
            None => unreachable!("checked flow selected a missing optional value"),
        });
    }
    evaluate_expression(head.clone(), context.clone())
}

pub(crate) fn evaluate_expression(
    expression: crate::node_modules::tsumo::engine::src::template::syntax::expressions::Expr,
    context: TemplateEvaluationContext,
) -> rt::TsonicResult<
    crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue,
> {
    if expression
        .dispatch
        .clone()
        .downcast_expr_to_token_expr()
        .is_some()
    {
        let token: String = js_string::trim(&{
            let dispatch_receiver = &{
                let downcast_value = &expression;
                crate::node_modules::tsumo::engine::src::template::syntax::expressions::TokenExpr {
                    identity: downcast_value.identity.clone(),
                    dispatch: downcast_value
                        .dispatch
                        .clone()
                        .downcast_expr_to_token_expr()
                        .unwrap(),
                }
            };
            dispatch_receiver.dispatch.read_token_expr_token()
        });
        if token == "."
            || token == "$"
            || js_string::starts_with_from_start(&token, ".")
            || js_string::starts_with_from_start(&token, "$")
            || js_string::starts_with_from_start(&token, "site")
            || token == "hugo.Sites"
            || js_string::starts_with_from_start(&token, "hugo.Sites.")
            || token == "hugo.Data"
            || js_string::starts_with_from_start(&token, "hugo.Data.")
            || token == "hugo.Store"
            || token == "resources"
            || token == "page"
            || js_string::starts_with_from_start(&token, "page.")
            || crate::node_modules::tsumo::engine::src::template::parser::tokens::PARSE_STRING_LITERAL
                .with(|module_binding| module_binding.load())
                .call((token.clone(),))?
                .is_some()
            || token == "true"
            || token == "false"
            || token == "nil"
            || crate::node_modules::tsumo::engine::src::template::evaluation::scalar_semantics::is_number_literal(
                token.clone(),
            )?
        {
            return crate::node_modules::tsumo::engine::src::template::evaluation::expression_semantics::eval_token(
                &token,
                context.state.with(|state| state.scope.clone()),
            );
        }
        return crate::node_modules::tsumo::engine::src::template::functions::call_function::call_template_function(
            token.clone(),
            js_abi::JsArray::from_dense(vec![]),
            context.state.with(|state| state.scope.clone()),
            context.state.with(|state| state.environment.clone()),
            context.state.with(|state| state.overrides.clone()),
            context.state.with(|state| state.defines.clone()),
        );
    }
    if expression
        .dispatch
        .clone()
        .downcast_expr_to_pipeline_expr()
        .is_some()
    {
        return evaluate_pipeline(
            {
                let dispatch_receiver_2 = &{
                    let downcast_value_2 = &expression;
                    crate::node_modules::tsumo::engine::src::template::syntax::expressions::PipelineExpr {
                        identity: downcast_value_2.identity.clone(),
                        dispatch: downcast_value_2
                            .dispatch
                            .clone()
                            .downcast_expr_to_pipeline_expr()
                            .unwrap(),
                    }
                };
                dispatch_receiver_2.dispatch.read_pipeline_expr_pipeline()
            },
            context.state.with(|state| state.scope.clone()),
            context.state.with(|state| state.environment.clone()),
            context.state.with(|state| state.overrides.clone()),
            context.state.with(|state| state.defines.clone()),
        );
    }
    if expression
        .dispatch
        .clone()
        .downcast_expr_to_command_expr()
        .is_some()
    {
        return evaluate_command(
            {
                let dispatch_receiver_3 = &{
                    let downcast_value_3 = &expression;
                    crate::node_modules::tsumo::engine::src::template::syntax::expressions::CommandExpr {
                        identity: downcast_value_3.identity.clone(),
                        dispatch: downcast_value_3
                            .dispatch
                            .clone()
                            .downcast_expr_to_command_expr()
                            .unwrap(),
                    }
                };
                dispatch_receiver_3.dispatch.read_command_expr_command()
            },
            context.clone(),
            Option::<crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue>::None,
        );
    }
    if expression
        .dispatch
        .clone()
        .downcast_expr_to_access_expr()
        .is_some()
    {
        let value: crate::node_modules::tsumo::engine::src::template::values::base::TemplateValue =
            evaluate_expression(
                {
                    let dispatch_receiver_4 = &{
                        let downcast_value_4 = &expression;
                        crate::node_modules::tsumo::engine::src::template::syntax::expressions::AccessExpr {
                            identity: downcast_value_4.identity.clone(),
                            dispatch: downcast_value_4
                                .dispatch
                                .clone()
                                .downcast_expr_to_access_expr()
                                .unwrap(),
                        }
                    };
                    dispatch_receiver_4.dispatch.read_access_expr_base()
                },
                context.clone(),
            )?;
        return crate::node_modules::tsumo::engine::src::template::evaluation::property_semantics::resolve_path(
            value.clone(),
            {
                let dispatch_receiver_5 = &{
                    let downcast_value_5 = &expression;
                    crate::node_modules::tsumo::engine::src::template::syntax::expressions::AccessExpr {
                        identity: downcast_value_5.identity.clone(),
                        dispatch: downcast_value_5
                            .dispatch
                            .clone()
                            .downcast_expr_to_access_expr()
                            .unwrap(),
                    }
                };
                dispatch_receiver_5.dispatch.read_access_expr_segments()
            },
            context.state.with(|state| state.scope.clone()),
        );
    }
    Err(rt::TsonicError::from(crate::node_modules::tsumo::engine::src::diagnostics::create_tsumo_error(
        String::from("TSUMO_TEMPLATE_EXPRESSION_INVALID"),
        String::from("The parsed template expression has no supported evaluation form"),
        None,
        None,
        None,
    )))
}
