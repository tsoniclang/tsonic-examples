...
{
    public static class Node_modules_Tsumo_engine_src_template_evaluation_pathSemantics
    {
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            {
                string normalized = Node_modules_Tsumo_engine_src_utils_strings.replaceText(raw, "\\", "/");
            ...
            {
                string slash = "/";
                string normalized = Node_modules_Tsumo_engine_src_template_evaluation_serialization.trimStartCharacter(Node_modules_Tsumo_engine_src_utils_strings.replaceText(Tsonic.CSharp.Js.String.trim(raw), "\\", "/"), slash);
                if (normalized == "")
                {
            ...
            resolvePageRef = (PageContext page, string @ref) =>
            {
                ...
                {
                    return Node_modules_Tsumo_engine_src_template_evaluation_serialization.trimSlashes(raw);
                }
                PageFile? pageFile = page.File;
                string @base = pageFile is not null ? pageFile.Dir : Node_modules_Tsumo_engine_src_template_evaluation_serialization.trimSlashes(page.relPermalink);
                string combined = @base == "" ? raw : Node_modules_Tsumo_engine_src_template_evaluation_serialization.trimEndCharacter(@base, "/") + "/" + Node_modules_Tsumo_engine_src_template_evaluation_serialization.trimStartCharacter(raw, "/");
            ...
            tryGetPage = (SiteContext site, string pathRaw) =>
            {
                ...
                string needle = Node_modules_Tsumo_engine_src_template_evaluation_serialization.trimSlashes(trimmed);
                if (needle == "")
                {
                    return site.home;
                }
                ...
                for (double i = 0; i < candidates.length; i++)
                {
                    ...
                    if (Node_modules_Tsumo_engine_src_template_evaluation_serialization.trimSlashes(p.relPermalink) == needle)
                    {
                        return p;
                    }
                    if (p.slug == needle)
