...
{
    public static class Node_modules_Tsumo_engine_src_template_parser_tokens
    {
        ...
        {
            Node_modules_Tsumo_engine_src_template_parser_stringLiterals.__tsonic_module_init();
            parseStringLiteral = (string token) => Node_modules_Tsumo_engine_src_template_parser_stringLiterals.decodeTemplateStringLiteral(token);
            ...
            scanTemplateSegments = (string template, string? sourcePath) =>
            {
                ...
                while (offset < source.length)
                {
                    ...
                    if (end < 0)
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_TEMPLATE_ACTION_UNCLOSED", "Template action opened with '{{' but has no closing '}}'", sourcePath, position_2.line, position_2.column);
                    }
            ...
            tokenizeTemplateAction = (string action, int? line, int? column, string? sourcePath) =>
            {
                ...
                while (offset < source.length)
                {
                    ...
                    if (character == "\"" || character == "'" || character == "`")
                    {
                        ...
                        {
                            throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_TEMPLATE_STRING_UNCLOSED", $"Template string opened with {quote} but is not closed", sourcePath, line, column);
                        }
                        offset++;
