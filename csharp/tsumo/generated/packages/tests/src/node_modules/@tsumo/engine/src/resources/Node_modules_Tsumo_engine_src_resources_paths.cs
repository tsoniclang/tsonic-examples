...
{
    public static class Node_modules_Tsumo_engine_src_resources_paths
    {
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            normalizeResourceRelativePath = (string path) =>
            {
                ...
                bool driveQualified = normalized.Length >= 2 && Node_modules_Tsumo_engine_src_utils_strings.substringCount(normalized, 1, 1) == ":";
                if (Tsonic.CSharp.Node.path.isAbsolute(normalized) || driveQualified)
                {
                    throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_RESOURCE_PATH_ABSOLUTE", $"Resource path must be source-root relative: {path}");
                }
                ...
                for (int index = 0; index < segments.length; index++)
                {
                    ...
                    if (segment == "..")
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_RESOURCE_PATH_ESCAPES_ROOT", $"Resource path escapes its root: {path}");
                    }
                    if (Tsonic.CSharp.Js.String.includes(segment, "\0"))
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_RESOURCE_PATH_INVALID", "Resource path contains a null character");
                    }
                    accepted.push(segment);
                }
                return Tsonic.CSharp.Js.Array.join(accepted, "/");
            };
            resourcePathToOsPath = (string relativePath) => Node_modules_Tsumo_engine_src_utils_strings.replaceText(relativePath, "/", $"{Tsonic.CSharp.Node.path.sep}");
            resolveContainedResourcePath = (string root, string relativePath) =>
            {
                ...
                if (!Node_modules_Tsumo_engine_src_utils_paths.pathContainsOrEquals(rootPath, candidate))
                {
                    throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_RESOURCE_PATH_ESCAPES_ROOT", $"Resource path escapes its root: {relativePath}");
                }
                return candidate;
            };
            splitResourcePath = (string relativePath) =>
            {
                ...
                return new ResourcePathParts(Node_modules_Tsumo_engine_src_utils_strings.substringCount(normalized, 0, index + 1), Node_modules_Tsumo_engine_src_utils_strings.substringFrom(normalized, index + 1));
            };
            splitResourceFileName = (string fileName) =>
            {
                ...
                return new ResourceFileNameParts(Node_modules_Tsumo_engine_src_utils_strings.substringCount(fileName, 0, index), Node_modules_Tsumo_engine_src_utils_strings.substringFrom(fileName, index));
            };
            return null;
        }
