...
{
    public static class Node_modules_Tsumo_engine_src_markdown_renderWithShortcodes
    {
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            Node_modules_Tsumo_engine_src_markdown_result.__tsonic_module_init();
            Node_modules_Tsumo_engine_src_markdown_pipeline.__tsonic_module_init();
            Node_modules_Tsumo_engine_src_markdown_toc.__tsonic_module_init();
            Node_modules_Tsumo_engine_src_markdown_renderHooks.__tsonic_module_init();
            Node_modules_Tsumo_engine_src_markdown_shortcodes.__tsonic_module_init();
            Node_modules_Tsumo_engine_src_markdown_renderBasic.__tsonic_module_init();
            ...
            protectStandardShortcodes = (string text, Tsonic.CSharp.Js.JSArray<ShortcodeCall> calls, PageContext page, SiteContext site, TemplateEnvironment env, ShortcodeOrdinalTracker ordinalTracker, Tsonic.CSharp.Js.Map<string, bool> recursionGuard) =>
            {
                ...
                {
                    outputs.push(Node_modules_Tsumo_engine_src_markdown_shortcodes.renderShortcode(calls[i], page, site, env, ordinalTracker, null, recursionGuard));
                }
                ...
                for (int i_3 = calls.length - 1; i_3 >= 0; i_3--)
                {
                    ...
                    source = Node_modules_Tsumo_engine_src_utils_strings.substringCount(source, 0, call.startIndex) + replacements[i_3].marker + Node_modules_Tsumo_engine_src_utils_strings.substringFrom(source, call.endIndex);
                }
            ...
            {
                string markdown = Node_modules_Tsumo_engine_src_markdown_renderBasic.normalizeNewlines(markdownRaw);
                ShortcodeOrdinalTracker ordinalTracker = Node_modules_Tsumo_engine_src_markdown_shortcodes.createOrdinalTracker();
                Tsonic.CSharp.Js.Map<string, bool> recursionGuard = new Tsonic.CSharp.Js.Map<string, bool>();
                Tsonic.CSharp.Js.JSArray<ShortcodeCall> calls = Node_modules_Tsumo_engine_src_shortcode.parseShortcodes(markdown, page.File?.Filename);
                ...
                if (mdCalls.length > 0)
                {
                    textAfterMarkdownShortcodes = Node_modules_Tsumo_engine_src_markdown_shortcodes.processShortcodeCalls(markdown, mdCalls, page, site, env, ordinalTracker, null, recursionGuard);
                }
                Tsonic.CSharp.Js.JSArray<ShortcodeCall> parsedStandardCalls = Node_modules_Tsumo_engine_src_shortcode.parseShortcodes(textAfterMarkdownShortcodes, page.File?.Filename);
                ...
                string toc = Node_modules_Tsumo_engine_src_markdown_toc.generateTableOfContents(markdownSource);
                RenderHookContext hookCtx = new RenderHookContext(page, site, env);
                int moreIndex = Node_modules_Tsumo_engine_src_markdown_renderBasic.findSummaryDividerIndex(markdownSource);
                string html = default(string)!;
                string summaryHtml = default(string)!;
                string plainText = default(string)!;
                if (moreIndex >= 0)
                {
                    string before = Node_modules_Tsumo_engine_src_utils_strings.substringCount(markdownSource, 0, moreIndex);
                    string after = Node_modules_Tsumo_engine_src_utils_strings.substringFrom(markdownSource, moreIndex + Node_modules_Tsumo_engine_src_markdown_renderBasic.summaryMarkerLength);
                    string full = before + after;
                    if (hookCtx.hasAnyHooks())
                    {
                        html = Node_modules_Tsumo_engine_src_markdown_renderHooks.renderMarkdownWithHooks(full, hookCtx);
                        summaryHtml = Tsonic.CSharp.Js.String.trim(Node_modules_Tsumo_engine_src_markdown_renderHooks.renderMarkdownWithHooks(before, hookCtx));
                    }
                    else
                    {
                        html = Markdig.Markdown.ToHtml(full, Node_modules_Tsumo_engine_src_markdown_pipeline.markdownPipeline);
                        summaryHtml = Tsonic.CSharp.Js.String.trim(Markdig.Markdown.ToHtml(before, Node_modules_Tsumo_engine_src_markdown_pipeline.markdownPipeline));
                    }
                    plainText = Markdig.Markdown.ToPlainText(full, Node_modules_Tsumo_engine_src_markdown_pipeline.markdownPipeline);
                }
                else
                {
                    if (hookCtx.hasAnyHooks())
                    {
                        html = Node_modules_Tsumo_engine_src_markdown_renderHooks.renderMarkdownWithHooks(markdownSource, hookCtx);
                    }
                    else
                    {
                        html = Markdig.Markdown.ToHtml(markdownSource, Node_modules_Tsumo_engine_src_markdown_pipeline.markdownPipeline);
                    }
                    plainText = Markdig.Markdown.ToPlainText(markdownSource, Node_modules_Tsumo_engine_src_markdown_pipeline.markdownPipeline);
                    string summarySource = Node_modules_Tsumo_engine_src_markdown_renderBasic.firstBlock(markdownSource);
                    if (summarySource == "")
                    {
                        summaryHtml = "";
                    }
                    else
                    {
                        if (hookCtx.hasAnyHooks())
                        {
                            summaryHtml = Tsonic.CSharp.Js.String.trim(Node_modules_Tsumo_engine_src_markdown_renderHooks.renderMarkdownWithHooks(summarySource, hookCtx));
                        }
                        else
                        {
                            summaryHtml = Tsonic.CSharp.Js.String.trim(Markdig.Markdown.ToHtml(summarySource, Node_modules_Tsumo_engine_src_markdown_pipeline.markdownPipeline));
                        }
                    }
                }
