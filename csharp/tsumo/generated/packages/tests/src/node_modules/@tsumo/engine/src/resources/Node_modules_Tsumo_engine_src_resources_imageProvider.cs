...
{
    public static class Node_modules_Tsumo_engine_src_resources_imageProvider
    {
        ...
        internal static int parsePositiveDimension(string value, string spec)
        {
            ...
            int? parsed = Node_modules_Tsumo_engine_src_utils_int32.parseInt32(value);
            if (parsed is null || parsed.Value <= 0)
            {
                throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_IMAGE_RESIZE_SPEC_INVALID", $"Invalid image resize specification: {spec}");
            }
            return parsed.Value;
        }
        internal static ImageResizeRequest parseImageResizeRequest(string spec)
        {
            ...
            if (tokens.length == 0)
            {
                throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_IMAGE_RESIZE_SPEC_INVALID", "Image resize specification cannot be empty");
            }
            ...
            for (double index = 1; index < tokens.length; index++)
            {
                ...
                if (token == "jpg" || token == "jpeg" || token == "png" || token == "gif" || token == "webp")
                {
                    ...
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_IMAGE_RESIZE_SPEC_INVALID", $"Image resize format is specified more than once: {spec}");
                    }
                    format = token == "jpeg" ? "jpg" : token;
                    continue;
                }
                throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_IMAGE_RESIZE_OPTION_UNSUPPORTED", $"Unsupported image resize option '{token}'");
            }
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            resizeImageResource = (Resource resource, string specification) =>
            {
                ...
                if (width <= 0 || height <= 0)
                {
                    throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_IMAGE_DIMENSIONS_UNKNOWN", "Image resizing with one automatic dimension requires known source dimensions");
                }
                ...
                if (sourceExtension == "")
                {
                    throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_IMAGE_FORMAT_UNKNOWN", "Image resizing requires a source file format");
                }
                ...
                try
                {
                    ...
                    ImageDimensions? dimensions = Node_modules_Tsumo_engine_src_resources_imageDimensions.parseImageDimensions(outputBytes);
                    ...
                    ResourcePathParts path = Node_modules_Tsumo_engine_src_resources_paths.splitResourcePath(outputRelPath);
                    ...
                    return new Resource($"{resource.id}|resize:{specification}", null, true, path.directory + outputFile, outputBytes, null, new ResourceData(""), Node_modules_Tsumo_engine_src_resources_mediaTypes.resourceMediaTypeForExtension(outputExtension), outputWidth, outputHeight);
                }
                finally
                {
