...
namespace Tsumo.Cli
{
    public static class Node_modules_Tsumo_engine_src_resources_pageBundle
    {
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            {
                paths.sort((string left, string right) => Node_modules_Tsumo_engine_src_utils_strings.compareText(left, right));
            };
            isNestedBundle = (string directory) => Node_modules_Tsumo_engine_src_fs.fileExists(Tsonic.CSharp.Node.path.join(directory, "index.md")) || Node_modules_Tsumo_engine_src_fs.fileExists(Tsonic.CSharp.Node.path.join(directory, "_index.md"));
            collectPageBundleResourceFiles = (string directory, string prefix, Tsonic.CSharp.Js.JSArray<PageBundleResourceFile> result) =>
            {
                Tsonic.CSharp.Js.JSArray<string> files = Node_modules_Tsumo_engine_src_fs.listFilesTopDirectory(directory, "*");
                sortPaths(files);
                ...
                Tsonic.CSharp.Js.JSArray<string> directories = Node_modules_Tsumo_engine_src_fs.listDirectoriesTopDirectory(directory);
                sortPaths(directories);
                for (int index_1 = 0; index_1 < directories.length; index_1++)
                {
                    string child = directories[index_1];
                    if (isNestedBundle(child) || Node_modules_Tsumo_engine_src_fs.listFilesTopDirectory(child, "*.md").length > 0)
                    {
                        continue;
                    }
