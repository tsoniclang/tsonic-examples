        ...
        private static object? __tsonic_module_init_core()
        {
            runExternalProcess = (string executable, Tsonic.CSharp.Js.JSArray<string> argumentsList, string toolName, string startDiagnosticCode) =>
            {
                ...
                if (error is not null || result.status is null)
                {
                    ...
                    throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError(startDiagnosticCode, detail == "" ? $"Failed to start {toolName} '{executable}'" : $"Failed to start {toolName} '{executable}': {detail}");
                }
