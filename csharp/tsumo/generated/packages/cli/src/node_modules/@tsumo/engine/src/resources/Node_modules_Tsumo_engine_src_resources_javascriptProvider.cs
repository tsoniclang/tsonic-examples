...
namespace Tsumo.Cli
{
    public static class Node_modules_Tsumo_engine_src_resources_javascriptProvider
    {
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            sourceExtension = (Resource resource) =>
            {
                ...
                string extension = Tsonic.CSharp.Js.String.toLowerCase(Node_modules_Tsumo_engine_src_resources_paths.splitResourceFileName(Node_modules_Tsumo_engine_src_resources_paths.splitResourcePath(raw).fileName).extension);
            ...
            outputRelativePath = (Resource resource, JavaScriptBuildOptions options) =>
            {
                ...
                ResourcePathParts path = Node_modules_Tsumo_engine_src_resources_paths.splitResourcePath(raw);
            ...
            {
                string sourceText = Node_modules_Tsumo_engine_src_resources_text.readResourceText(resource, "js.Build");
                if (options.sourceMap != "none")
                {
                    throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_JAVASCRIPT_SOURCE_MAP_UNSUPPORTED", "js.Build currently supports only sourceMap 'none'");
                }
                ...
                try
                {
                    ...
                    if (sourcePath is not null && Node_modules_Tsumo_engine_src_fs.fileExists(sourcePath) && Node_modules_Tsumo_engine_src_fs.readTextFile(sourcePath) == sourceText)
                    {
                    ...
                    ExternalProcessResult process = Node_modules_Tsumo_engine_src_resources_externalProcess.runExternalProcess(executable, argumentsList, "esbuild", "TSUMO_ESBUILD_START_FAILED");
                    if (process.exitCode != 0)
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_ESBUILD_FAILED", process.standardError == "" ? $"esbuild failed with exit code {process.exitCode}" : process.standardError);
                    }
                    if (!Tsonic.CSharp.Node.fs.existsSync(outputPath))
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_ESBUILD_OUTPUT_MISSING", "esbuild completed without producing JavaScript");
                    }
    ...
    public class JavaScriptBuildOptions
    {
        ...
        public string cacheKey()
        {
            ...
            {
                result += Node_modules_Tsumo_engine_src_resources_javascriptProvider.cacheKeyPart(values[index]);
            }
            return result;
        }
    }
