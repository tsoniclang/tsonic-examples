...
namespace Tsumo.Cli
{
    public static class Node_modules_Tsumo_engine_src_docs_markdown
    {
        ...
        internal static string? maybeRewriteUrl(string? urlValue, DocsLinkRewriteContext ctx)
        {
            ...
            if (isUnsafeUrl(url))
            {
                throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_DOCS_LINK_UNSAFE", $"Unsafe docs link: {url}", ctx.sourcePath);
            }
            if (url == "" || Tsonic.CSharp.Js.String.startsWith(url, "#") || isExternalUrl(url))
            {
                return null;
            }
            UrlSuffixSplit split = Node_modules_Tsumo_engine_src_docs_url.splitUrlSuffix(url);
            ...
            if (Tsonic.CSharp.Js.String.startsWith(pathPart, "/"))
            {
                ...
                else
                {
                    ...
                    {
                        resolvedRel = Node_modules_Tsumo_engine_src_utils_strings.trimStartChar(Node_modules_Tsumo_engine_src_utils_strings.substringFrom(pathPart, ctx.mount.urlPrefix.Length), slash);
                    }
                    else
                    {
                        return null;
            ...
            if (escaped)
            {
                if (ctx.strictLinks)
                {
                    throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_DOCS_LINK_ESCAPES_MOUNT", $"Out-of-mount link from {ctx.mount.name}: {url}", ctx.sourcePath);
                }
                ...
                string repoPath = Node_modules_Tsumo_engine_src_utils_strings.trimEndChar(Node_modules_Tsumo_engine_src_utils_strings.trimStartChar(Tsonic.CSharp.Js.String.trim(repoPathRaw), slash), slash);
            ...
            if (ctx.strictLinks)
            {
                throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_DOCS_LINK_UNRESOLVED", $"Unresolved docs link from {ctx.mount.name}: {url}", ctx.sourcePath);
            }
            return null;
        }
        ...
        {
            return Node_modules_Tsumo_engine_src_utils_strings.replaceLineEndings(text, "\n");
        }
        ...
        {
            return Node_modules_Tsumo_engine_src_utils_strings.indexOfTextIgnoreCase(markdown, summaryMarker);
        }
        internal static string firstBlock(string markdown)
        {
            ...
            int idx = Node_modules_Tsumo_engine_src_utils_strings.indexOfText(text, "\n\n");
            return idx >= 0 ? Node_modules_Tsumo_engine_src_utils_strings.substringCount(text, 0, idx) : text;
        }
        internal static string renderWithRewrites(string markdown, DocsLinkRewriteContext ctx)
        {
            Markdig.Syntax.MarkdownDocument doc = Markdig.Markdown.Parse(markdown, Node_modules_Tsumo_engine_src_markdown_pipeline.markdownPipeline);
            rewriteLinks(doc, ctx);
            return Markdig.Markdown.ToHtml(doc, Node_modules_Tsumo_engine_src_markdown_pipeline.markdownPipeline);
        }
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            renderDocsMarkdown = (string markdownRaw, DocsLinkRewriteContext ctx) =>
            {
                ...
                if (moreIndex >= 0)
                {
                    string before = Node_modules_Tsumo_engine_src_utils_strings.substringCount(markdown, 0, moreIndex);
                    string after = Node_modules_Tsumo_engine_src_utils_strings.substringFrom(markdown, moreIndex + summaryMarkerLength);
                    string full = before + after;
                    return new MarkdownResult(renderWithRewrites(full, ctx), Tsonic.CSharp.Js.String.trim(renderWithRewrites(before, ctx)), Markdig.Markdown.ToPlainText(full, Node_modules_Tsumo_engine_src_markdown_pipeline.markdownPipeline), "");
                }
                ...
                return new MarkdownResult(html, summaryHtml, Markdig.Markdown.ToPlainText(markdown, Node_modules_Tsumo_engine_src_markdown_pipeline.markdownPipeline), "");
            };
            return null;
        }
