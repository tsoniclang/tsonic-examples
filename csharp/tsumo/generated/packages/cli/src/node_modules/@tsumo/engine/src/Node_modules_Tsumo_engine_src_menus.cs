...
namespace Tsumo.Cli
{
    public static class Node_modules_Tsumo_engine_src_menus
    {
        ...
        internal static void assertAcyclicParents(Tsonic.CSharp.Js.JSArray<MenuEntry> entries, Tsonic.CSharp.Js.Map<string, MenuEntry> byIdentity)
        {
            for (double index = 0; index < entries.length; index++)
            {
                ...
                while (Tsonic.CSharp.Js.String.trim(current.parent) != "")
                {
                    ...
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_MENU_PARENT_CYCLE", $"Menu parent cycle includes '{identity}'");
                    }
                    ...
                    if (parent is null)
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_MENU_PARENT_NOT_FOUND", $"Menu entry '{identity}' names missing parent '{current.parent}'");
                    }
                    current = parent;
                }
            }
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            compareMenuEntries = (MenuEntry left, MenuEntry right) =>
            {
                ...
                int identity = Node_modules_Tsumo_engine_src_utils_strings.compareText(menuEntryIdentity(left), menuEntryIdentity(right));
                if (identity != 0)
                {
                    return identity;
                }
                int name = Node_modules_Tsumo_engine_src_utils_strings.compareText(left.name, right.name);
                return name != 0 ? name : Node_modules_Tsumo_engine_src_utils_strings.compareText(left.url, right.url);
            };
            buildMenuHierarchy = (Tsonic.CSharp.Js.JSArray<MenuEntry> entries) =>
            {
                ...
                for (double index = 0; index < entries.length; index++)
                {
                    ...
                    if (identity == "")
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_MENU_IDENTITY_REQUIRED", "Every menu entry requires an identifier or name");
                    }
                    if (byIdentity.has(identity))
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_MENU_IDENTITY_DUPLICATE", $"Duplicate menu entry identity: {identity}");
                    }
                ...
                for (double index_1 = 0; index_1 < entries.length; index_1++)
                {
                    ...
                    else
                    {
                        ...
                        if (parent is null)
                        {
                            throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_MENU_PARENT_NOT_FOUND", $"Menu entry '{menuEntryIdentity(entry_1)}' names missing parent '{parentName}'");
                        }
