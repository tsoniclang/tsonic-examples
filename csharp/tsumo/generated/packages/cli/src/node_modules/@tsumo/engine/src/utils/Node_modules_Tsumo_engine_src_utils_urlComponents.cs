...
namespace Tsumo.Cli
{
    public static class Node_modules_Tsumo_engine_src_utils_urlComponents
    {
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            decodeUrlComponent = (string value) =>
            {
                ...
                while (index < value.Length)
                {
                    ...
                    {
                        appendUtf8(decoded, Node_modules_Tsumo_engine_src_utils_strings.substringCount(value, literalStart, index - literalStart));
                    }
                    if (index + 2 >= value.Length)
                    {
                        throw new System.Exception("URL component contains an incomplete percent escape");
                    }
                    int high = hexValue(Node_modules_Tsumo_engine_src_utils_strings.substringCount(value, index + 1, 1));
                    int low = hexValue(Node_modules_Tsumo_engine_src_utils_strings.substringCount(value, index + 2, 1));
                    if (high < 0 || low < 0)
                    {
                ...
                {
                    appendUtf8(decoded, Node_modules_Tsumo_engine_src_utils_strings.substringCount(value, literalStart, value.Length - literalStart));
                }
