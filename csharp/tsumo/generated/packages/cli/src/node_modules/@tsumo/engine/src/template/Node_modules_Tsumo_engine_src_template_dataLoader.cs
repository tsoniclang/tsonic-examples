...
namespace Tsumo.Cli
{
    public static class Node_modules_Tsumo_engine_src_template_dataLoader
    {
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            normalizeDataPath = (string path) => Node_modules_Tsumo_engine_src_utils_strings.replaceText(path, "\\", "/");
            collectDataLayer = (string root, Tsonic.CSharp.Js.Map<string, SelectedDataFile> selected) =>
            {
                if (!Node_modules_Tsumo_engine_src_fs.dirExists(root))
                {
                    return;
                }
                Tsonic.CSharp.Js.JSArray<string> files = Node_modules_Tsumo_engine_src_fs.listFilesRecursive(root, "*");
                ...
                for (int index = 0; index < files.length; index++)
                {
                    ...
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_DATA_IDENTITY_CONFLICT", $"Data files '{existing.sourcePath}' and '{sourcePath}' define the same data identity '{semanticPath}'", sourcePath);
                    }
            ...
            setDataPath = (DictValue root, string semanticPath, TemplateValue value, string sourcePath) =>
            {
                ...
                for (int index = 0; index < segments.length - 1; index++)
                {
                    ...
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_DATA_TREE_CONFLICT", $"Data identity '{semanticPath}' conflicts with a data file at '{Tsonic.CSharp.Js.Array.join(Tsonic.CSharp.Js.Array.slice(segments, 0, index + 1), "/")}'", sourcePath);
                    }
                ...
                {
                    throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_DATA_TREE_CONFLICT", $"Data identity '{semanticPath}' is declared more than once", sourcePath);
                }
            ...
            loadSiteData = (string siteDir, string? themeDir, Tsonic.CSharp.Js.JSArray<ModuleMount>? mounts) =>
            {
                ...
                if (mounts is not null)
                {
                    for (int index = mounts.length - 1; index >= 0; index--)
                    {
                        ...
                        string target = Node_modules_Tsumo_engine_src_utils_strings.trimEndChar(Node_modules_Tsumo_engine_src_utils_strings.trimStartChar(normalizeDataPath(mount.target), "/"), "/");
                        if (target != "data")
                        {
                            continue;
                        }
                ...
                for (int index_1 = 0; index_1 < identities.length; index_1++)
                {
                    ...
                    if (file is null)
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_DATA_SELECTION_INCONSISTENT", $"Selected data identity '{identity}' disappeared");
                    }
                    TemplateValue value = Node_modules_Tsumo_engine_src_template_evaluation_structuredData.parseTemplateDataText(Node_modules_Tsumo_engine_src_fs.readTextFile(file.sourcePath), file.format, file.sourcePath);
