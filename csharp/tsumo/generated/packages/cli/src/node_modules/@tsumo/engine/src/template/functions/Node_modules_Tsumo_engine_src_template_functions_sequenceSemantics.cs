...
namespace Tsumo.Cli
{
    public static class Node_modules_Tsumo_engine_src_template_functions_sequenceSemantics
    {
        ...
        internal static int sequenceArgument(TemplateValue value, int position)
        {
            ...
            {
                int? parsed = Node_modules_Tsumo_engine_src_utils_int32.parseInt32(((StringValue)value).value);
                if (parsed is not null)
                {
                    return parsed.Value;
                }
            }
            throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_TEMPLATE_SEQUENCE_ARGUMENT_INVALID", $"seq argument {position} must be a 32-bit integer");
        }
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            createIntegerSequence = (Tsonic.CSharp.Js.JSArray<TemplateValue> args) =>
            {
                ...
                {
                    throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_TEMPLATE_SEQUENCE_ARGUMENT_INVALID", "seq requires one, two, or three integer arguments");
                }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_TEMPLATE_SEQUENCE_INCREMENT_INVALID", "seq increment must be non-zero and move from the first value toward the last value");
                        }
                    }
                }
                double difference = last - first;
                double sizeValue = Tsonic.CSharp.Js.Math.floor(difference / increment) + 1;
                int? size = Node_modules_Tsumo_engine_src_utils_int32.toInt32(sizeValue);
                if (size is null || size.Value <= 0 || size.Value > maxSequenceSize)
                {
                    throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_TEMPLATE_SEQUENCE_SIZE_UNSUPPORTED", $"seq cannot produce more than {maxSequenceSize} values");
                }
                ...
                for (int index = 0; index < size.Value; index++)
                {
                    ...
                    {
                        int? next = Node_modules_Tsumo_engine_src_utils_int32.toInt32(value + increment);
                        if (next is null)
                        {
                            throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_TEMPLATE_SEQUENCE_SIZE_UNSUPPORTED", "seq result exceeds the supported 32-bit integer range");
                        }
                        value = next.Value;
                    }
                }
