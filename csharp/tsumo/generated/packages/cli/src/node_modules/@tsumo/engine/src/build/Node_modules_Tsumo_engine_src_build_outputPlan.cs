...
namespace Tsumo.Cli
{
    public static class Node_modules_Tsumo_engine_src_build_outputPlan
    {
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            {
                string normalized = Node_modules_Tsumo_engine_src_build_siteRoutes.normalizeSitePath(relativePath);
                ...
                {
                    throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_OUTPUT_PATH_ABSOLUTE", $"Site output path must be relative: {relativePath}");
                }
                Tsonic.CSharp.Js.JSArray<string> segments = Node_modules_Tsumo_engine_src_build_siteRoutes.splitSitePath(normalized);
                for (int index = 0; index < segments.length; index++)
                {
                    string segment = segments[index];
                    if (segment == "" || segment == "." || segment == "..")
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_OUTPUT_PATH_ESCAPES_ROOT", $"Site output path is not canonical: {relativePath}");
                    }
                }
                return Node_modules_Tsumo_engine_src_build_siteRoutes.joinSitePath(segments);
            };
            combineOutputPath = (string prefix, string relativePath) =>
            {
                ...
                return normalizeOutputPath(Node_modules_Tsumo_engine_src_build_siteRoutes.normalizeSitePath(prefix) + "/" + normalizedRelativePath);
            };
            resolveOutputPath = (string outputRoot, string relativePath) =>
            {
                ...
                if (!Node_modules_Tsumo_engine_src_utils_paths.pathContainsOrEquals(root, candidate))
                {
                    throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_OUTPUT_PATH_ESCAPES_ROOT", $"Site output path escapes its root: {relativePath}");
                }
                return candidate;
            };
            return null;
    ...
    public class SiteOutputPlan
    {
        ...
        {
            string outputPath = Node_modules_Tsumo_engine_src_build_outputPlan.normalizeOutputPath(relativePath);
        ...
        {
            Tsonic.CSharp.Js.JSArray<string> files = Node_modules_Tsumo_engine_src_fs.listFilesRecursive(sourceRoot, "*");
            ...
            for (int index = 0; index < files.length; index++)
            {
                ...
                string relativePath = Node_modules_Tsumo_engine_src_build_siteRoutes.normalizeSitePath(Tsonic.CSharp.Node.path.relative(sourceRoot, sourcePath));
                this.addAsset(Node_modules_Tsumo_engine_src_build_outputPlan.combineOutputPath(outputPrefix, relativePath), sourcePath, owner, layer);
            }
        }
        ...
        public void applyDeferredTemplateResults(Tsonic.CSharp.Js.Map<string, string> results)
        {
            ...
            for (int outputIndex = 0; outputIndex < outputPaths.length; outputIndex++)
            {
                ...
                foreach (string token in results.keys())
                {
                    ...
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_TEMPLATE_DEFER_RESULT_INVALID", "A deferred-template replacement disappeared");
                    }
                    ...
                    if (first >= 0)
                    {
                        ...
                        {
                            throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_TEMPLATE_DEFER_PLACEMENT_INVALID", "Each deferred-template placement must occur exactly once in planned output");
                        }
        ...
        public void render(string outputRoot)
        {
            ...
            keys.sort((string left, string right) => Node_modules_Tsumo_engine_src_build_siteRoutes.compareSitePaths(left, right));
            for (int index = 0; index < keys.length; index++)
            {
                ...
                if (claim is null)
                {
                    throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_OUTPUT_PLAN_INCONSISTENT", $"Output claim '{key}' disappeared before rendering");
                }
                string destination = Node_modules_Tsumo_engine_src_build_outputPlan.resolveOutputPath(outputRoot, claim.relativePath);
                string? text = Tsonic.CSharp.Js.Map.getReference<string, string>(this.textByPath, key);
                if (text is not null)
                {
                    Node_modules_Tsumo_engine_src_fs.writeTextFile(destination, text);
                    continue;
                }
                FileSiteOutput? file = Tsonic.CSharp.Js.Map.getReference<string, FileSiteOutput>(this.filesByPath, key);
                if (file is null)
                {
                    throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_OUTPUT_PLAN_INCONSISTENT", $"Output claim '{key}' has no planned content");
                }
                Node_modules_Tsumo_engine_src_fs.ensureDir(Tsonic.CSharp.Node.path.dirname(destination));
        ...
        {
            throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_OUTPUT_PATH_CONFLICT", $"Output '{relativePath}' is claimed by both '{previous.owner}' and '{owner}'");
        }
    }
}
