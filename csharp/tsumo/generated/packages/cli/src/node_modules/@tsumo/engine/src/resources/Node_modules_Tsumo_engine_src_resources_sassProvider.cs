...
namespace Tsumo.Cli
{
    public static class Node_modules_Tsumo_engine_src_resources_sassProvider
    {
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            {
                string sourceText = Node_modules_Tsumo_engine_src_resources_text.readResourceText(resource, "css.Sass");
                ...
                {
                    throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_SASS_IMPLEMENTATION_INVALID", $"Unsupported Sass implementation '{implementation}'; expected 'dart-sass' or 'libsass'");
                }
                ...
                try
                {
                    ...
                    for (double index = 0; index < loadPaths.length; index++)
                    {
                        ...
                        if (!Node_modules_Tsumo_engine_src_fs.dirExists(loadPath))
                        {
                            continue;
                        }
                    ...
                    ExternalProcessResult process = Node_modules_Tsumo_engine_src_resources_externalProcess.runExternalProcess(executable, argumentsList, "Sass compiler", "TSUMO_SASS_START_FAILED");
                    if (process.exitCode != 0)
                    {
                        string stderr = process.standardError;
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_SASS_FAILED", stderr == "" ? $"Sass compiler failed with exit code {process.exitCode}" : stderr);
                    }
                    if (!Tsonic.CSharp.Node.fs.existsSync(outputPath))
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_SASS_OUTPUT_MISSING", "Sass compiler completed without producing CSS");
                    }
                    string text = Tsonic.CSharp.Node.fs.readFileSync(outputPath, "utf8");
                    string outputPathRaw = resource.outputRelPath ?? "style.scss";
                    ResourcePathParts path = Node_modules_Tsumo_engine_src_resources_paths.splitResourcePath(outputPathRaw);
