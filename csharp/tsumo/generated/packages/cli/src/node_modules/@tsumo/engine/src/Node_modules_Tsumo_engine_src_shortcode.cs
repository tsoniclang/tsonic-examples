...
namespace Tsumo.Cli
{
    public static class Node_modules_Tsumo_engine_src_shortcode
    {
        internal static string parseQuotedString(ParseState state, string? sourcePath, int line, int column)
        {
            ...
            if (!closed)
            {
                throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_SHORTCODE_STRING_UNCLOSED", $"Shortcode string opened with {quote} but is not closed", sourcePath, line, column);
            }
            return result;
        }
        ...
        internal static ObjectShape_6409d8cac3e5 parseParams(string argsText, string? sourcePath, int line, int column)
        {
            ...
            while (!state.atEnd())
            {
                ...
                if (foundEquals)
                {
                    if (key == "")
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_SHORTCODE_PARAMETER_INVALID", "Shortcode named parameters require a name", sourcePath, line, column);
                    }
                    if (@params.has(key))
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_SHORTCODE_PARAMETER_DUPLICATE", $"Shortcode parameter '{key}' is declared more than once", sourcePath, line, column);
                    }
                    isNamed = true;
                    state.skipWhitespace();
                    if (state.atEnd())
                    {
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_SHORTCODE_PARAMETER_INVALID", $"Shortcode parameter '{key}' requires a value", sourcePath, line, column);
                    }
            ...
            {
                throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_SHORTCODE_PARAMETER_STYLE_MIXED", "Shortcode parameters cannot mix named and positional forms", sourcePath, line, column);
            }
            return new ObjectShape_5338cd423f9f
            {
        ...
        internal static ObjectShape_ec37db37c8f1? findClosingTag(string text, string name, int startPos, bool isMarkdown)
        {
            ...
            {
                string remaining = Node_modules_Tsumo_engine_src_utils_strings.substringFrom(text, pos);
                if (Tsonic.CSharp.Js.String.startsWith(remaining, openTag))
                {
                    string afterOpen = Tsonic.CSharp.Js.String.trimStart(Node_modules_Tsumo_engine_src_utils_strings.substringFrom(text, pos + openTag.Length));
                ...
                if (Tsonic.CSharp.Js.String.startsWith(remaining, closeTagPrefix) || Tsonic.CSharp.Js.String.startsWith(remaining, closeTagPrefix2))
                {
                    ...
                    if (depth == 0)
                    {
                        string inner = Node_modules_Tsumo_engine_src_utils_strings.substringCount(text, innerStart, pos - innerStart);
                        string endSuffix = isMarkdown ? "%}}" : ">}}";
                        int closeEnd = Node_modules_Tsumo_engine_src_utils_strings.indexOfTextFrom(text, endSuffix, pos);
                        if (closeEnd < 0)
                        {
                            return null;
                        }
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            parseShortcodes = (string text, string? sourcePath) =>
            {
                ...
                while (pos < text.Length)
                {
                    int openAngle = Node_modules_Tsumo_engine_src_utils_strings.indexOfTextFrom(text, "{{<", pos);
                    int openPercent = Node_modules_Tsumo_engine_src_utils_strings.indexOfTextFrom(text, "{{%", pos);
                    int openPos = -1;
                    ...
                    int closePos = Node_modules_Tsumo_engine_src_utils_strings.indexOfTextFrom(text, closeSuffix, openPos + 3);
                    if (closePos < 0)
                    {
                        ShortcodePosition position = sourceMap.positionAt(openPos);
                        throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_SHORTCODE_ACTION_UNCLOSED", $"Shortcode action opened with '{(isMarkdown ? "{{%" : "{{<")}' but is not closed", sourcePath, position.line, position.column);
                    }
                    string content = Tsonic.CSharp.Js.String.trim(Node_modules_Tsumo_engine_src_utils_strings.substringCount(text, openPos + 3, closePos - (openPos + 3)));
                    bool isSelfClosing = Tsonic.CSharp.Js.String.endsWith(content, "/");
                    string tagContent = isSelfClosing ? Tsonic.CSharp.Js.String.trim(Node_modules_Tsumo_engine_src_utils_strings.substringCount(content, 0, content.Length - 1)) : content;
                    ...
                    string name = firstSpace >= 0 ? Tsonic.CSharp.Js.String.trim(Node_modules_Tsumo_engine_src_utils_strings.substringCount(tagContent, 0, firstSpace)) : Tsonic.CSharp.Js.String.trim(tagContent);
                    string argsText = firstSpace >= 0 ? Node_modules_Tsumo_engine_src_utils_strings.substringFrom(tagContent, firstSpace + 1) : "";
                    if (name == "" || Tsonic.CSharp.Js.String.startsWith(name, "/"))
                    {
                        if (Tsonic.CSharp.Js.String.startsWith(name, "/"))
                        {
                            ...
                            throw Node_modules_Tsumo_engine_src_diagnostics.createTsumoError("TSUMO_SHORTCODE_CLOSE_UNEXPECTED", $"Unexpected shortcode closing action '{name}'", sourcePath, position_1.line, position_1.column);
                        }
                        pos = closePos + closeSuffix.Length;
                        continue;
                    }
                    ShortcodePosition position_2 = sourceMap.positionAt(openPos);
                    ObjectShape_6409d8cac3e5 parsed = parseParams(argsText, sourcePath, position_2.line, position_2.column);
            ...
            innerDeindent = (string inner) =>
            {
                ...
                for (int i = 0; i < lines.length; i++)
                {
                    ...
                    {
                        string c = Node_modules_Tsumo_engine_src_utils_strings.substringCount(line, j, 1);
                        if (c == " ")
                        {
                            indent++;
                        }
                ...
                for (int i_1 = 0; i_1 < lines.length; i_1++)
                {
                    ...
                    {
                        string c_1 = Node_modules_Tsumo_engine_src_utils_strings.substringCount(line_1, j_1, 1);
                        if (c_1 == " ")
                        {
                            removed++;
                            startIdx++;
                    ...
                    result.push(Node_modules_Tsumo_engine_src_utils_strings.substringFrom(line_1, startIdx));
                }
    ...
    public class ParseState
    {
        ...
        public string peek(int offset)
        {
            ...
            return idx < this.text.Length ? Node_modules_Tsumo_engine_src_utils_strings.substringCount(this.text, idx, 1) : "";
        }
        public string peekString(int length)
        {
            ...
            return Node_modules_Tsumo_engine_src_utils_strings.substringCount(this.text, this.pos, sliceLength);
        }
