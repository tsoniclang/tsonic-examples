...
namespace Tsumo.Cli
{
    public static class Node_modules_Tsumo_engine_src_watchSnapshot
    {
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            createWatchSnapshot = (Tsonic.CSharp.Js.JSArray<string> targets) =>
            {
                ...
                for (int i = 0; i < targets.length; i++)
                {
                    ...
                    if (Node_modules_Tsumo_engine_src_fs.fileExists(target))
                    {
                        addFileState(snapshot, target);
                        continue;
                    }
                    if (!Node_modules_Tsumo_engine_src_fs.dirExists(target))
                    {
                        continue;
                    }
                    Tsonic.CSharp.Js.JSArray<string> files = Node_modules_Tsumo_engine_src_fs.listFilesRecursive(target, "*");
