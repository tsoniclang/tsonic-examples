...
namespace Tsumo.Engine
{
    ...
    {
        internal static bool isRightParenToken(string token)
        {
    ...
    public class PipelineParser
    {
        ...
        {
            Tsonic.CSharp.Js.JSArray<Command> stages = Tsonic.CSharp.Js.JSArray<Command>.of([]);
        ...
        public Command parseCommand()
        {
            ...
            if ((object?)head is TokenExpr && Tsonic.CSharp.Js.String.toLowerCase(Tsonic.CSharp.Js.String.trim(((TokenExpr)head).token)) == "return")
            {
                if (this.index >= this.tokens.length || this.tokens[this.index] == "|" || this.tokens[this.index] == ")")
                {
                    return new Command((TokenExpr)head, Tsonic.CSharp.Js.JSArray<Expr>.of([]));
                }
                return new Command((TokenExpr)head, Tsonic.CSharp.Js.JSArray<Expr>.of([new CommandExpr(this.parseCommand())]));
            }
            Tsonic.CSharp.Js.JSArray<Expr> args = Tsonic.CSharp.Js.JSArray<Expr>.of([]);
            while (this.index < this.tokens.length)
            {
                string token = this.tokens[this.index];
