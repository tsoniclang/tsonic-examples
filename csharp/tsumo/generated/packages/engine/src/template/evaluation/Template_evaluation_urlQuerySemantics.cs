...
namespace Tsumo.Engine
{
    ...
    {
        internal static bool isHexDigit(string value)
        {
            double code = Tsonic.CSharp.Js.String.charCodeAt(value, 0);
            return (code >= 48 && code <= 57) || (code >= 65 && code <= 70) || (code >= 97 && code <= 102);
        }
        internal static string decodeQueryComponent(string value)
        {
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            parseUrlQuery = (string rawQuery) =>
            {
                ...
                for (double index = 0; index < fields.length; index++)
                {
                    ...
                    if (existing is null)
                    {
                        values.set(name, Tsonic.CSharp.Js.JSArray<string>.of([value]));
                    }
                    else
                    {
                        existing.push(value);
