...
namespace Tsumo.Engine
{
    ...
    {
        internal static TsumoError yamlError(string message, string? sourcePath, int line)
        {
            return Diagnostics.createTsumoError("TSUMO_TEMPLATE_UNMARSHAL_YAML_INVALID", message, sourcePath, line, 1);
        }
        internal static int yamlSourceIndentation(string raw, string? sourcePath, int line)
        {
            int indentation = 0;
        ...
        internal static int yamlMappingSeparator(string value)
        {
            string quote = "";
            bool escaped = false;
        ...
        internal static int? yamlQuotedScalarStart(string content)
        {
            int start = 0;
        ...
        internal static YamlQuoteScan scanYamlQuotedScalar(string content, int quoteStart, string quote)
        {
        ...
        internal static YamlLogicalLine readYamlLogicalLine(Tsonic.CSharp.Js.JSArray<string> sourceLines, int sourceIndex, int indent, string? sourcePath)
        {
        ...
        internal static TemplateValue jsonToTemplateValue(JsonValue value)
        {
            ...
            if ((object?)value is JsonArray)
            {
                ...
                for (double index = 0; index < ((JsonArray)value).items.length; index++)
                {
            ...
            if ((object?)value is JsonObject)
            {
                ...
                for (double index_1 = 0; index_1 < ((JsonObject)value).properties.length; index_1++)
                {
        ...
        internal static StructuredInput inputFromValue(TemplateValue value)
        {
        ...
        internal static string? optionValue(DictValue options, string name)
        {
        ...
        internal static string normalizeFormat(string? requested, StructuredInput input)
        {
    ...
    public class YamlTemplateParser
    {
        ...
        {
            this.lines = Tsonic.CSharp.Js.JSArray<YamlLine>.of([]);
            this.sourcePath = sourcePath;
                ...
                if (item != "")
                {
                    ...
                    if (separator >= 0)
                    {
                        ...
                        if (current < this.lines.length && this.lines[current].indent > indent)
                        {
                            ...
                            if (!((object?)continuation.value is DictValue))
                            {
        ...
        public YamlParseResult parseBlockScalar(YamlBlockScalarHeader header, int parentIndent, int headerLine, int nextParsedIndex)
        {
            ...
            Tsonic.CSharp.Js.JSArray<int> indentations = Tsonic.CSharp.Js.JSArray<int>.of([]);
