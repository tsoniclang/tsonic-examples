...
namespace Tsumo.Engine
{
    ...
    {
        internal static string renderPartialResolution(PartialTemplateResolution selected, TemplateValue contextValue, TemplateFunctionContext context)
        {
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            callTemplateFunctionFamily = (string name, Tsonic.CSharp.Js.JSArray<TemplateValue> args, TemplateFunctionContext context) =>
            {
                ...
                if (name == "templates.defer" && args.length == 1 && (object?)args[0] is DictValue)
                {
                    ...
                    if (keyValue is not null && !((object?)keyValue is StringValue))
                    {
                        throw Diagnostics.createTsumoError("TSUMO_TEMPLATE_DEFER_KEY_INVALID", "templates.Defer key must be a string");
                    }
                    string? key = keyValue is not null ? ((StringValue)keyValue).value : null;
                ...
                if (name == "path.join" && args.length >= 1)
                {
                    ...
                    bool rooted = false;
                    for (double argumentIndex = 0; argumentIndex < args.length; argumentIndex++)
                    {
                        ...
                        for (double partIndex = 0; partIndex < parts.length; partIndex++)
                        {
