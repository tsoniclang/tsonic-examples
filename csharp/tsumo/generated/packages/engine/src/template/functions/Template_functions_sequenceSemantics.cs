...
namespace Tsumo.Engine
{
    public static class Template_functions_sequenceSemantics
    {
        ...
        internal static int sequenceArgument(TemplateValue value, int position)
        {
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            createIntegerSequence = (Tsonic.CSharp.Js.JSArray<TemplateValue> args) =>
            {
                ...
                if (args.length == 1)
                {
                    if (last == 0)
                    {
                        return new AnyArrayValue(Tsonic.CSharp.Js.JSArray<TemplateValue>.of([]));
                    }
                    if (last > 0)
                    {
                        first = 1;
            ...
            reverseTemplateCollection = (TemplateValue collection) =>
            {
                ...
                if ((object?)collection is StringArrayValue)
                {
                    Tsonic.CSharp.Js.JSArray<string> result_1 = Tsonic.CSharp.Js.JSArray<string>.of([]);
