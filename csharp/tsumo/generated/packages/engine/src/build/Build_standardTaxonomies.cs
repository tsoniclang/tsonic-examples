...
namespace Tsumo.Engine
{
    ...
    {
        internal static StandardTaxonomy createTaxonomyPage(StandardPageGraph graph, string taxonomy, Tsonic.CSharp.Js.Map<string, Tsonic.CSharp.Js.JSArray<PageContext>> pagesByTerm)
        {
            ...
            PageContext root = new PageContext(Utils_text.humanizeSlug(taxonomy), "", "", false, "taxonomy", taxonomy, taxonomy, taxonomy, Utils_urlPath.combineUrlPath(Tsonic.CSharp.Js.JSArray<string>.of([taxonomy])), "", emptyHtml, emptyHtml, emptyHtml, "", emptyStrings, emptyStrings, taxonomyParameters, null, graph.site.Language, emptyPages, null, graph.site, emptyPages, graph.home, Tsonic.CSharp.Js.JSArray<PageContext>.of([graph.home]), null);
            ...
            Tsonic.CSharp.Js.JSArray<PageContext> terms = Tsonic.CSharp.Js.JSArray<PageContext>.of([]);
            for (double index = 0; index < termSlugs.length; index++)
            {
                ...
                PageContext term = new PageContext(Utils_text.humanizeSlug(termSlug), "", "", false, "term", taxonomy, taxonomy, termSlug, Utils_urlPath.combineUrlPath(Tsonic.CSharp.Js.JSArray<string>.of([taxonomy, termSlug])), "", new HtmlString(""), new HtmlString(""), new HtmlString(""), "", emptyStrings, emptyStrings, parameters, null, graph.site.Language, emptyPages, null, graph.site, termPages, root, Tsonic.CSharp.Js.JSArray<PageContext>.of([graph.home, root]), null);
                terms.push(term);
            }
            root.pages = terms;
            Tsonic.CSharp.Js.Map<string, PageContext> termPages_1 = new Tsonic.CSharp.Js.Map<string, PageContext>();
            for (double index_1 = 0; index_1 < terms.length; index_1++)
            {
        ...
        internal static Tsonic.CSharp.Js.Map<string, Tsonic.CSharp.Js.JSArray<PageContext>> collectTerms(Tsonic.CSharp.Js.JSArray<PageContext> pages, Func<PageContext, Tsonic.CSharp.Js.JSArray<string>> selectTerms)
        {
            ...
            for (double pageIndex = 0; pageIndex < pages.length; pageIndex++)
            {
                ...
                for (double termIndex = 0; termIndex < terms.length; termIndex++)
                {
                    ...
                    Tsonic.CSharp.Js.JSArray<PageContext> termPages = Tsonic.CSharp.Js.Map.getReference<string, Tsonic.CSharp.Js.JSArray<PageContext>>(pagesByTerm, termSlug) ?? Tsonic.CSharp.Js.JSArray<PageContext>.of([]);
                    termPages.push(page);
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            createStandardTaxonomies = (StandardPageGraph graph) =>
            {
                ...
                Tsonic.CSharp.Js.JSArray<StandardTaxonomy> taxonomies = Tsonic.CSharp.Js.JSArray<StandardTaxonomy>.of([createTaxonomyPage(graph, "tags", tagsByTerm), createTaxonomyPage(graph, "categories", categoriesByTerm)]);
                Tsonic.CSharp.Js.JSArray<PageContext> allPages = Tsonic.CSharp.Js.JSArray<PageContext>.of([]);
                for (double index = 0; index < graph.site.allPages.length; index++)
                {
                    allPages.push(graph.site.allPages[index]);
                }
                for (double taxonomyIndex = 0; taxonomyIndex < taxonomies.length; taxonomyIndex++)
                {
                    StandardTaxonomy taxonomy = taxonomies[taxonomyIndex];
                    allPages.push(taxonomy.root);
                    for (double termIndex = 0; termIndex < taxonomy.terms.length; termIndex++)
                    {
