...
namespace Tsumo.Engine
{
    ...
    {
        internal static bool matchesPattern(string filePath, string searchPattern)
        {
        ...
        internal static Tsonic.CSharp.Js.JSArray<ManagedDirectoryEntry> listManagedDirectoryEntries(string directory)
        {
            if (!dirExists(directory))
            {
                return Tsonic.CSharp.Js.JSArray<ManagedDirectoryEntry>.of([]);
            }
            rejectFilesystemLink(directory);
            string[] names = Tsonic.CSharp.Node.fs.readdirSync(directory);
            Tsonic.CSharp.Js.JSArray<ManagedDirectoryEntry> entries = Tsonic.CSharp.Js.JSArray<ManagedDirectoryEntry>.of([]);
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            {
                Tsonic.CSharp.Js.JSArray<ManagedDirectoryEntry> entries = listManagedDirectoryEntries(rootDir);
                Tsonic.CSharp.Js.JSArray<string> directories = Tsonic.CSharp.Js.JSArray<string>.of([]);
                for (double index = 0; index < entries.length; index++)
                {
                    ManagedDirectoryEntry entry = entries[index];
                    if (entry.directory)
