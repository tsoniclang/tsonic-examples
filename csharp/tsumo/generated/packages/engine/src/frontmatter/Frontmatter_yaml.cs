        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            parseYamlFrontMatter = (Tsonic.CSharp.Js.JSArray<string> lines, string? sourcePath) =>
            {
                ...
                while (index < lines.length)
                {
                    ...
                    string text = yamlText(raw);
                    if (text == "" || Tsonic.CSharp.Js.String.startsWith(text, "#"))
                    {
                        index++;
                        continue;
                    }
                    ...
                    {
                        throw Diagnostics.createTsumoError("TSUMO_FRONTMATTER_YAML_SYNTAX_INVALID", "YAML front matter has an unexpected indented entry", sourcePath, lineNumber, 1);
                    }
                    Tsonic.CSharp.Js.JSArray<string> pair = splitYamlPair(text, sourcePath, lineNumber);
                    string key = pair[0];
                    string value = pair[1];
                    Frontmatter_scalars.recordFrontMatterField(rootFields, key, "Front matter", sourcePath, lineNumber);
                    if (value != "")
                    {
                        Frontmatter_scalars.applyFrontMatterScalar(frontMatter, key, value, "yaml", sourcePath, lineNumber);
                        index++;
                        continue;
                    }
                    string normalizedKey = Tsonic.CSharp.Js.String.toLowerCase(key);
                    index++;
                    if (normalizedKey == "params")
                    {
                        ...
                        while (index < lines.length && indentationOf(lines[index]) > 0)
                        {
                            ...
                            if (childText != "" && !Tsonic.CSharp.Js.String.startsWith(childText, "#"))
                            {
                                ...
                                {
                                    throw Diagnostics.createTsumoError("TSUMO_FRONTMATTER_YAML_SYNTAX_INVALID", "Front matter params require one scalar mapping level", sourcePath, childLine, 1);
                                }
                                Tsonic.CSharp.Js.JSArray<string> child = splitYamlPair(childText, sourcePath, childLine);
                                if (child[1] == "")
                                {
                                    throw Diagnostics.createTsumoError("TSUMO_FRONTMATTER_PARAM_INVALID", $"Front matter param '{child[0]}' requires a scalar value", sourcePath, childLine, 1);
                                }
                                Frontmatter_scalars.recordFrontMatterField(paramFields, child[0], "Front matter params", sourcePath, childLine);
                                frontMatter.Params.set(child[0], Frontmatter_scalars.parseFrontMatterParam(child[1], "yaml", sourcePath, childLine));
                            }
                            index++;
                        }
                        continue;
                    ...
                    if (normalizedKey == "tags" || normalizedKey == "categories")
                    {
                        ...
                        while (index < lines.length && indentationOf(lines[index]) > 0)
                        {
                            ...
                            int childLine_1 = index + 2;
                            validateYamlLine(childRaw_1, sourcePath, childLine_1);
                            string childText_1 = yamlText(childRaw_1);
                            if (childText_1 != "" && !Tsonic.CSharp.Js.String.startsWith(childText_1, "#"))
                            {
                                if (indentationOf(childRaw_1) != 2 || !Tsonic.CSharp.Js.String.startsWith(childText_1, "-"))
                                {
                                    throw Diagnostics.createTsumoError("TSUMO_FRONTMATTER_INVALID_STRING_ARRAY", $"Front matter field '{key}' requires a scalar list", sourcePath, childLine_1, 1);
                                }
                                string item = Tsonic.CSharp.Js.String.trim(Utils_strings.substringFrom(childText_1, 1));
                                if (item == "")
                                {
                                    throw Diagnostics.createTsumoError("TSUMO_FRONTMATTER_INVALID_STRING_ARRAY", $"Front matter field '{key}' contains an empty list item", sourcePath, childLine_1, 1);
                                }
                                values.push(Frontmatter_scalars.parseFrontMatterString(item, key, "yaml", sourcePath, childLine_1));
                            }
                            index++;
                        }
                    ...
                    if (normalizedKey == "menu")
                    {
                        ...
                        while (index < lines.length && indentationOf(lines[index]) > 0)
                        {
                            ...
                            {
                                throw Diagnostics.createTsumoError("TSUMO_FRONTMATTER_YAML_SYNTAX_INVALID", "Front matter menu names require one mapping level", sourcePath, entryLine, 1);
                            }
                            Tsonic.CSharp.Js.JSArray<string> entryPair = splitYamlPair(entryText, sourcePath, entryLine);
                            if (entryPair[1] != "")
                            {
                                throw Diagnostics.createTsumoError("TSUMO_FRONTMATTER_MENU_INVALID", $"Front matter menu '{entryPair[0]}' requires a property mapping", sourcePath, entryLine, 1);
                            }
                            Frontmatter_scalars.recordFrontMatterField(menuNames, entryPair[0], "Front matter menu", sourcePath, entryLine);
                            FrontMatterMenu entry = new FrontMatterMenu(entryPair[0]);
                            ...
                            {
                                string propertyRaw = lines[index];
                                int propertyLine = index + 2;
                                validateYamlLine(propertyRaw, sourcePath, propertyLine);
                                string propertyText = yamlText(propertyRaw);
                                if (propertyText != "" && !Tsonic.CSharp.Js.String.startsWith(propertyText, "#"))
                                {
                                    if (indentationOf(propertyRaw) != 4)
                                    {
                                        throw Diagnostics.createTsumoError("TSUMO_FRONTMATTER_YAML_SYNTAX_INVALID", "Front matter menu properties require exactly two mapping levels", sourcePath, propertyLine, 1);
                                    }
                                    Tsonic.CSharp.Js.JSArray<string> property = splitYamlPair(propertyText, sourcePath, propertyLine);
                                    if (property[1] == "")
                                    {
                                        throw Diagnostics.createTsumoError("TSUMO_FRONTMATTER_MENU_INVALID", $"Front matter menu field '{property[0]}' requires a scalar value", sourcePath, propertyLine, 1);
                                    }
                                    Frontmatter_scalars.recordFrontMatterField(menuFields, property[0], $"Front matter menu '{entry.menu}'", sourcePath, propertyLine);
                    ...
                    throw Diagnostics.createTsumoError("TSUMO_FRONTMATTER_NESTED_VALUE_UNSUPPORTED", $"Front matter field '{key}' does not support a nested value", sourcePath, lineNumber, 1);
                }
                return frontMatter;
            };
            return null;
