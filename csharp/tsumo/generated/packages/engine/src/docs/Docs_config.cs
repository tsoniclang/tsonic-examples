...
namespace Tsumo.Engine
{
    ...
    {
        internal static TsumoError docsConfigError(string code, string message, string path)
        {
            return Diagnostics.createTsumoError(code, message, path);
        }
        internal static void assertUniqueProperties(JsonObject value, string context, string path)
        {
        ...
        internal static string? optionalString(JsonObject root, string name, string context, string path)
        {
        ...
        internal static bool? optionalBool(JsonObject root, string name, string context, string path)
        {
            JsonValue? value = root.getCaseInsensitive(name);
            if (value is null)
            {
        ...
        internal static string requiredString(JsonObject root, string name, string context, string path)
        {
        ...
        internal static void rejectUnknownProperties(JsonObject root, Tsonic.CSharp.Js.JSArray<string> allowedNames, string context, string path)
        {
            Tsonic.CSharp.Js.Map<string, bool> allowed = new Tsonic.CSharp.Js.Map<string, bool>();
            for (double index = 0; index < allowedNames.length; index++)
            {
                allowed.set(Tsonic.CSharp.Js.String.toLowerCase(allowedNames[index]), true);
            }
            for (double index_1 = 0; index_1 < root.properties.length; index_1++)
            {
        ...
        {
            return Utils_text.ensureTrailingSlash(Utils_text.ensureLeadingSlash(Tsonic.CSharp.Js.String.trim(raw)));
        }
        internal static string resolveSourceDir(string siteDir, string raw, string path)
        {
        ...
        internal static DocsMountConfig parseMount(string siteDir, JsonValue value, double index, string path)
        {
            ...
            rejectUnknownProperties(@object, Tsonic.CSharp.Js.JSArray<string>.of(["name", "source", "prefix", "repoUrl", "repoBranch", "repoPath", "navPath"]), context, path);
        ...
        internal static Tsonic.CSharp.Js.JSArray<DocsMountConfig> parseMounts(string siteDir, JsonObject root, string path)
        {
            ...
            Tsonic.CSharp.Js.JSArray<DocsMountConfig> mounts = Tsonic.CSharp.Js.JSArray<DocsMountConfig>.of([]);
        ...
        private static object? __tsonic_module_init_core()
        {
            ...
            loadDocsConfig = (string siteDir) =>
            {
                ...
                rejectUnknownProperties(root, Tsonic.CSharp.Js.JSArray<string>.of(["siteName", "homeMount", "strictLinks", "search", "searchFile", "mounts"]), "tsumo.docs.json", candidate);
